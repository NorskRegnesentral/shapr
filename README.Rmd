...
(Shapley Additive Global importancE) values, computed by setting `sage = TRUE` in `explain()`.
...
(Shapley Additive Global importancE) values by setting `sage = TRUE` and providing the observed responses via
`y_explain`. The loss defaults to log-loss for binary 0/1 responses and the mean squared error otherwise, and a
custom loss can be supplied through `sage_args`.
