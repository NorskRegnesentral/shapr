...
joint_csv_filename <- "all_results_dim10_Gauss_Gaussmix.csv" # Set to NULL if results should not be included in any joint results table

initial_current_csv_filename <- "Experiment_7_Linear_Linear_Gaussian"
true_model <- "Linear"
fitted_model <- "Linear"
variables <- "Gaussian"
...
mu.list = list(rep(0,10))
mat <- matrix(rho,ncol=10,nrow=10)
diag(mat) <- 1
Sigma.list <- list(mat)
