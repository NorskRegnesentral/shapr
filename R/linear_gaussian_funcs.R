...
prepare_data.linear_gaussian <- function(internal, index_features = NULL, ...) {
  ...
  for (i in seq_len(nrow(S))){
    ...
    QSbar_list[[i]] <- t(PSbar[[i]])%*%PSbar[[i]] # Make this every time as well since it is fast to do, even though it could be extracted from QS_list if it is always present.
  }
