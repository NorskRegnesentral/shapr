...
#' @export
predict_model.ar <- function(x, newdata) {
  if (!requireNamespace("stats", quietly = TRUE)) {
    stop("The stats package is required for predicting stats models")
  }

  as.data.frame(t(apply(newdata, 1, function (n) predict(x, rev(n), n.ahead = x$n.ahead, se.fit = FALSE))))
}
...
get_model_specs.ar <- function(x) {
  ...
  feature_specs <- list()
  feature_specs$labels <- paste0("ar", seq_along(x$ar))
