...
#' @param approach_marginal Character. Either `independence` (default) or `gaussian`. This decides if the marginal
#' MC samples are generated by sampling the training data or sampling from a Gaussian distribution. We currently
#' only use the former option, and their is no way for the user to change `approach_marginal`. We then have to
#' make `approach_marginal` into a parameter in the `explain()` function.
#' TODO: discuss with Martin if we should do this, otherwise we can remove the option here. Can then also do it for
#' gaussian copula. Maybe have it that if `approach = 'gaussian'`, then we sample from marginal gaussian, but
#' for all other approaches, we use the `independence` setup.
...
prepare_data_causal <- function(internal, index_features = NULL, approach_marginal = "independence", ...) {
  ...
  approach_marginal = internal$parameters$approach_marginal
  ...
  for (index_feature_idx in seq_along(index_features)) {
    ...
    for (sampling_step_idx in seq_along(S_causal_now)) {
      ...
      if (is.null(S_now)) {
        ...
        # Generate the marginal data either form the training data or from the Gaussian distribution.
        if (approach_marginal == "independence") {
          dt_Sbar_now_marginal_values = create_marginal_data(x_train = x_train,
                                                             n_explain = n_explain,
                                                             Sbar_features = Sbar_now,
                                                             n_samples = n_samples)
        } else if (approach_marginal == "gaussian") {
          if (approach != "gaussian") stop("`approach` must be `gaussian` when `approach_marginal = gaussian`.")
          ...
          stop("SHAPR currently only supports `independence and ``gaussian` as the `approach_marginal`.")
        }
