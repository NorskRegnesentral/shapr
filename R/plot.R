...
plot.shapr <- function(x,
  ...
  if(plot_type=="scatter"){
    ...
    gg <- gg + geom_point(aes(x=feature_value, y=phi), colour=col[1]) + #need to document that the color is specified like his
      ...
      if(all(col %in% c("#00BA38","#F8766D")) & length(col)==2){ #check is col-parameter is the default
        gg<-gg +
          scale_color_gradient2(low=col[2],mid="yellow", high = col[1], midpoint = 0.5, breaks = c(0,1), labels=c("Low", "High"), name="Feature\nvalue")
      }
      else if(length(col)==3){ # allow user to specify three colors
        gg<-gg +
          scale_color_gradient2(low=col[3], mid=col[2], high = col[1], midpoint=0.5, breaks = c(0,1), labels=c("Low", "High"), name="Feature\nvalue")
