...
plot.shapr <- function(x,
  ...
  if (is.null(index_x_test)) index_x_test <- seq(nrow(x$explainer$x_test))
  if (is.null(top_k_features)) top_k_features <- ncol(x$explainer$x_test) + 1
  ...
  if (!is_groupwise) {
    desc_mat <- format(x$explainer$x_test, digits = digits)
