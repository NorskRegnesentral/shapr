...
plot_kshap <- function(explanation,
  ...
  # melting Kshap
  KshapDT <- data.table::copy(explanation)
  KshapDT[, id := .I]
  ...
  preds <- predict_model(model, Xtest)
  predDT <- data.table::data.table(id = KshapDT$id, pred = preds)
...
#' TODO: DELETE THIS OLDER VERSION
#'
#'
#' Plot of the Shapley value explanations
#'
...
#'
#' @author Martin Jullum
plot_kshap_old <- function(explanation,
                       Xtest,
                       no_desc_digits = 3,
                       plot_phi0 = T,
                       plot_which_Xtest = 1:nrow(Xtest),
