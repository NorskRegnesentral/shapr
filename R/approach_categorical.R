...
prepare_data.categorical <- function(internal, index_features = NULL, ...) {
  ...
  if (!all(round(check) == 1)) {
    cli::cli_warn("Not all marginal probabilities sum to 1. There could be a problem
  ...
  if (!all(round(check) == 1)) {
    cli::cli_warn("Not all conditional probabilities sum to 1. There could be a problem
            with the joint probabilities. Consider checking.")
  }
