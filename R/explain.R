...
#' If `TRUE`, SAGE values are computed and can be found as `shapley_values_est` in the output.
#' The computation of SAGE values is based on
#' \href{https://proceedings.neurips.cc/paper/2020/file/c7bf0b7c1a86d5eb3be2c722cf2cf746-Paper.pdf}{
#' Covert et al. (2020)}, sampling from conditional distributions rather than marginal as described by Covert et. al.
#' If iterative is also `TRUE`, the convergence will be in regards to the SAGE values.
#' Regular Shapley values can be found under `internal$output` in all cases.
#'
#' @param response Numerical vector.
#' Not applicable unless the `sage` paramter is set to `TRUE`.
