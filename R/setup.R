...
check_data <- function(internal) {
  ...
  if (is.null(model_feature_specs)) {
    if ("basic" %in% verbose) {
      msg1 <- paste0("You passed a model to explain() which is not natively supported, and did not supply a ",
...
check_extra_computation_args <- function(extra_computation_args) {
  ...
  if (!(length(vS_batching_method) == 1 && vS_batching_method %in%
    c("future", "forloop"))) {
    cli::cli_abort("`vS_batching_method` must be one of `future`, `forloop`.\n")
  }
