...
#' @param mask_generator_only_these_coalitions_probabilities Numerics containing the probabilities
  ...
  initialize = function(one_hot_max_sizes,
    ...
    if (mask_generator_name == "MCAR_mask_generator") {
      ...
      if (is.null(mask_generator_only_these_coalitions) | is.null(mask_generator_only_these_coalitions_probabilities)) {
        stop("Both 'mask_generator_only_these_coalitions' and 'mask_generator_only_these_coalitions_probabilities'
...
#' A torch::nn_module Representing a GaussianCategoricalSampler
#'
#' @description
#' The GaussianCategoricalSampler generates a sample from the generative distribution defined by
...
#'
#' @return
#' A `GaussianCategoricalSampler` object.
#'
#' @author Lars Henry Berge Olsen
#' @keywords internal
GaussianCategoricalSampler <- torch::nn_module(
  ...
  # @description Initialize a GaussianCategoricalSampler which generates a sample from the generative
...
#' A torch::nn_module Representing a GaussianCategoricalSamplerRandom
#'
#' @description
#' The GaussianCategoricalSamplerRandom generates random samples from the generative
...
GaussianCategoricalSamplerRandom <- torch::nn_module(
  ...
  # @description
  # Initialize a GaussianCategoricalSamplerRandom which generates a sample from the
  # generative distribution defined by the output of the neural network by random sampling.
  # @return A new `GaussianCategoricalSamplerRandom` object.
  initialize = function(one_hot_max_sizes,
                        min_sigma = 1e-4,
                        min_prob = 1e-4) {
    # one-hot max size of i-th feature, if i-th feature is categorical,
...
#' A torch::nn_module Representing a GaussianCategoricalParameters
#'
#' @description
#' The GaussianCategoricalParameters module extracts the parameters
...
GaussianCategoricalParameters <- torch::nn_module(
  ...
  # @description
  # Initialize a `GaussianCategoricalParameters` which extract the parameters from the
  # generative distribution defined by the output of the neural network.
  # @return A new `GaussianCategoricalParameters` object.
  initialize = function(one_hot_max_sizes,
                        min_sigma = 1e-4,
                        min_prob = 1e-4) {
    # one-hot max size of i-th feature, if i-th feature is categorical,
...
#' A torch::nn_module Representing a GaussianCategoricalLoss
#'
#' @description
#' The GaussianCategoricalLoss module/layer computes the log probability
...
GaussianCategoricalLoss <- torch::nn_module(
  ...
  # @description Initialize a `GaussianCategoricalLoss`.
  # @return A new `GaussianCategoricalLoss` object.
