...
vaeac_normalize_data = function(data_torch, one_hot_max_sizes, norm_mean = NULL, norm_std = NULL) {
  ...
  # Return the values
  list(data_normalized_torch = data_normalized_torch,
       norm_mean = norm_mean,
       norm_std = norm_std)
}
...
#' @param vaeac_model_state_list List. The returned list from the `vaeac_preprocess_data` function or a loaded list of a saved vaeac object.
#'
...
#' #' print(data_unbounded)
#' #' }
#' vaeac_log_trans_cont_features <- function(data, one_hot_max_sizes) {
...
#' #' data_unbounded <- vaeac_log_trans_cont_features(data, one_hot_max_sizes)
#' #' print(data_unbounded)
#' #' data_bounded <- vaeac_exp_trans_cont_features(data_unbounded, one_hot_max_sizes)
#' #' print(data_bounded)
...
#' #' all.equal(data, data_bounded)
#' #' }
#' vaeac_exp_trans_cont_features <- function(data, one_hot_max_sizes) {
#'   # Check if data is data.table or matrix/data.frame.
#'   if (any(class(data) == "data.table")) {
#'     # Extract which columns that are continuous.
#'     cont_cols <- seq_along(one_hot_max_sizes)[one_hot_max_sizes < 2]
