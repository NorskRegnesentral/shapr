...
#' @param xtest Matrix, data.frame or data.table with the features, whose predictions ought
...
#' @param no_samp Integer. How many samples to use when approximating the sum in the Shapley
#' formula
...
#' Defaults to "empirical_fixed_sigma", with "empirical_AICc_full", "empirical_AICc_each_k",
#' "Gaussian" and "copula" being other alternatives. One can also supply a named list where the
#' names are one or more of the implemented methods, and the named lists contains one vector each,
#' each containing the row numbers of the S-matrix computed using \code{prepare_kshap} that whose
#' corresponding conditional expectations should be computed with that method. Any number not
...
#'  conditional distribution. Defaults to "Euclidean", "Mahalanobis" and "Mahalanobis_scaled"
#'  being the other options. "Mahalanobis_scaled" includes
...
#' Defaults to "Gaussian" [\eqn{\exp(-D/2\sigma)}], with "independence" (imputing independently,
#' ignoring any distance) being the second option
