...
prepare_data.independence <- function(internal, index_features = NULL, ...) {
  ...
  feature_spec <- internal$objects$feature_spec
  ...
  non_numeric_features <- feature_spec$labels[feature_spec$classes != "numeric"]
  ...
  for (i in seq(n_col)) {
    ...
    data.table::setnames(dt_l[[i]], feature_spec$labels)
