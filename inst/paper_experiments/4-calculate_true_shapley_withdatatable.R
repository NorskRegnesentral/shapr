...
cond_prob <- function(marg_list, joint_prob_dt, explainer, model=NULL, group=NULL){
  ...
  if(is_groupwise){
    ...
    group_all_num <- lapply(group_all_names, FUN = function(x){match(x, feature_labels)})

    dt_combinations_all <- shapr:::feature_combinations(
      m = explainer$n_features,
      exact = explainer$exact,
      n_combinations = NULL,
      weight_zero_m = 10^6,
      group_num = group_all_num
    )
    ...
    id_combination <- NULL
    for(i in 1:nrow(dt_combinations)){
      indices <- c(indices, which(dt_combinations_all[['features']] %in% dt_combinations[['features']][i]))
    }
    ...
    for(i in 2:nrow(explainer$S)){
      ...
      mat0 <- marg_list[[i]]
      #feat_names0 <- head(names(marg_list[[i]]),-1)
      #which(feat_names %in% feat_names0)
      ...
      cond_list[[i]] <- cbind(cond_list[[i]], dt_combinations_all[i, c('features')])
      cond_list[[i]][, (feat_names):= NULL] # To save memory
      if(i %in% indices){
        cond_list[[i]][, id_combination := which(indices == i)]
        cond_list[[i]][, groups := dt_combinations[id_combination,][['groups']]]

      } else{
        cond_list[[i]][, id_combination := NA]
        cond_list[[i]][, groups := NA]
...
cond_expec_new <- function(cond_list, explainer, x_test, prediction_zero, joint_prob_dt, group_names=NULL){
  ...
  is_groupwise <- !is.null(group_names)
  if(is_groupwise){
    tmp <- list()
    tmp0 <- NULL
    for(i in 2:nrow(explainer$S)){
    ...
    S_char_vec <- tail(names(final_dt), -1)
    final_dt_x_test <- cbind("0" = prediction_zero,
                             final_dt[x_test_id, ..S_char_vec, on = "rowid"])
    names(final_dt_x_test) <- as.character(group_names)
