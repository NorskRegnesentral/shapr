...
# pre_grouped = pre_grouped_stand2
# post_grouped = post_grouped_stand2
...
groupA_shapley = fread("inst/paper_experiments/results/group1_Shapley_values_GAM.csv")
groupA2_shapley = fread("inst/paper_experiments/results/groupA_Shapley_values_GAM.csv")
...
results = list()
for(exper in c("experiment_gam3")  ){
  for(corr in c(0, 0.1, 0.3)){
    ...
    MAD0 = MAD(pre_grouped, post_grouped)

    MAD1 = MAD(pre_grouped_stand2, post_grouped_stand2)

    MAD2 = MAD(pre_grouped_stand3, post_grouped_stand3)

    MDR0 = MDR(pre_grouped_rank, post_grouped_rank)

    results[[length(results) + 1]] = data.frame(exper, corr, MAD_no_stand = MAD0, MAD_stand = MAD1, MAD_avg_sd_stand = MAD2, MDR0)
...
# Half way through I started saving the gam results with another name :/
groupB_shapley = fread("inst/paper_experiments/results/group2_Shapley_values_GAM.csv")
groupB2_shapley = fread("inst/paper_experiments/results/groupB_Shapley_values_GAM.csv")
