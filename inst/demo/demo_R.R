...
expl_default_gaussian <- explain(model = model,
                                 x_explain = x_explain,
                                 x_train = x_train,
                                 approach = "gaussian",
                                 phi0 = phi0)
...
expl_ctree <- explain(model = model,
                              x_explain = x_explain,
                              x_train = x_train,
                              approach = "ctree",
...
expl_ctree_higher_tol <- explain(model = model,
                              x_explain = x_explain,
                              x_train = x_train,
                              approach = "ctree",
                              verbose = c("basic", "progress", "convergence", "vS_details"),
                              phi0 = phi0,
                              seed = 123,
                              iterative_args = list(convergence_tol = 0.01,
                                                    n_coal_next_iter_factor_vec = rep(0.8,10)))


expl_ctree_40 <- explain(model = model,
                        x_explain = x_explain,
                        x_train = x_train,
                        approach = "ctree",
                        phi0 = phi0,
...
expl_ctree_40_MCsamp_100 <- explain(model = model,
                        x_explain = x_explain,
                        x_train = x_train,
                        approach = "ctree",
                        phi0 = phi0,
...
expl_copula_40 <- explain(model = model,
                                 x_explain = x_explain,
                                 x_train = x_train,
                                 approach = "copula",
                                 max_n_coalitions = 40,
...
## Separate
exp_reg_sep_40 <- explain(model = model,
                           x_explain = x_explain,
                           x_train = x_train,
                           phi0 = phi0,
...
exp_reg_sep_40_tuned <- explain(model = model,
                           x_explain = x_explain,
                           x_train = x_train,
                           phi0 = phi0,
                           approach = "regression_separate",
...
## Surrogate
exp_reg_surr_40 <- explain(model = model,
                           x_explain = x_explain,
                           x_train = x_train,
                           phi0 = phi0,
...
plot_MSEv_eval_crit(list(ctree_40 = expl_ctree_40,
...
expl_copula_60 <- explain(model = model,
                                 x_explain = x_explain,
                                 x_train = x_train,
                                 approach = "copula",
                                 max_n_coalitions = 60,
...
expl_vaeac <- explain(model = model,
...
plot(expl_default_gaussian,
     plot_type = "bar",
     index_x_explain = c(1,22,45,89),
     bar_plot_phi0 = FALSE)

plot(expl_default_gaussian, plot_type = "waterfall",index_x_explain  = c(1,22,45,89))

plot(expl_default_gaussian, plot_type = "scatter")

plot(expl_default_gaussian, plot_type = "scatter", scatter_features = c("trend","temp"))

plot(expl_default_gaussian, plot_type = "beeswarm")

plot(expl_default_gaussian, plot_type = "beeswarm", col = c("pink","blue"))
...
expl_group_ctree <- explain(model = model,
                     x_explain = x_explain,
                     x_train = x_train,
                     phi0 = phi0,
                     group = group,
...
expl_group_vaeac <- explain(model = model,
                     x_explain = head(x_explain),
                     x_train = x_train,
                     phi0 = phi0,
                     group = group,
...
for (i in seq_along(expl_list)) {
  plot_list[[i]] <- plot(expl_list[[i]],
                         plot_type = "beeswarm",
                         print_ggplot = FALSE) +
    ggplot2::ggtitle(names(expl_list)[i]) + ggplot2::ylim(-3700, 3700)
}
...
model_arimax <- arima(x_full[seq_len(700), temp],
                      order = c(2, 1, 1),
