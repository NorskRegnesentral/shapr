...
Hallmark_genes = qusage::read.gmt("~/nr/project_stat/BI_PersonalisedMarketing/Explanations/Martin/groupShapley-paper/gene_data/h.all.v7.4.symbols.gmt") # HALLMARK


colitis = GEOquery::getGEO(GEO = "GPL96",filename = "~/nr/project_stat/BI_PersonalisedMarketing/Explanations/Martin/groupShapley-paper/gene_data/GDS1615_full.soft.gz")
...
# Define gene sets being used (in practice), i.e. has nonzero betas
used_genesets_glmnet0 <-  unique(geneset_dt[data.table(gene=nonzero_betas),on="gene"]) # A total of 48 genes
geneset_dt0 <- copy(geneset_dt)
geneset_dt0[,gene_set_N:=.N,by=gene_set]

# Assigning the genes in the model with non-zero betas to the gene set with the largest number og genes overall
...
# We only use genes in these gene sets. However, the genes in these gene sets may also be present in multiple gene sets

# Here we assign the genes to the gene set with the largest number of genes
...
save(list=object_list,file = "inst/groupSHAP/shapley_objects_new.RData")
