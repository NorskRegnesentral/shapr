...
predict_model <- function(x,newdata) predict(x, newdata,num.threads = 1)$predictions
if(current_predictive_function_name=="gam") predict_model = mgcv::predict.bam

current_training_MSE = mean((as.matrix(predict_model(current_model, current_data$training_df)) - current_data$training_y)^2)
current_test_MSE = mean((as.matrix(predict_model(current_model, current_data$test_df)) - current_data$test_y)^2)
...
# Parallel shapr, and arf trian, but not arf gen
...
# Parallel shapr, and arf train, and arf gen
expl <- shapr::explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
...
# Parallel shapr, not arf train, and arf gen
expl <- shapr::explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
...
# Sequential shapr, parallel arf train, and arf gen
expl <- shapr::explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
