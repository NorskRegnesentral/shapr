...
simulate_data <- function(parameters_list){
  ...
  explanation_list[['empirical_ind_nsamples100']]$dt_sum <- cbind(explanation_list[['empirical_ind_nsamples100']]$dt[, 1], phi_sum_mat)
  ...
  colnames(phi_sum_mat) <- feat_names
  explanation_list[['empirical_nsamples1000']]$dt_sum <- cbind(explanation_list[['empirical_nsamples1000']]$dt[, 1], phi_sum_mat)
  ...
  colnames(phi_sum_mat) <- feat_names
  explanation_list[['gaussian_nsamples100']]$dt_sum <- cbind(explanation_list[['gaussian_nsamples100']]$dt[, 1], phi_sum_mat)
  ...
  for (i in 1:no_features){
    phi_sum_mat[, i] <- rowSums(subset(explanation_list[['gaussian_nsamples1000']]$dt, select = which(beta_matcher == i) + 1))
  }
  colnames(phi_sum_mat) <- feat_names
  explanation_list[['gaussian_nsamples1000']]$dt_sum <- cbind(explanation_list[['gaussian_nsamples1000']]$dt[, 1], phi_sum_mat)
