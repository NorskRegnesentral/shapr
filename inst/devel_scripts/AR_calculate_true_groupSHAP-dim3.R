...
group2 <- list(c(1, 3, 2)) # two groups: group1 and group2
...
set.seed(10) # need to re-do this or you get errors
joint_prob_dt_list <- create_exact_joint_prob(mu, Sigma, beta, explainer, cutoff, response_mod)
marg_list <- marg_prob(joint_prob_dt_list[[1]], explainer)
...
head(cbind(Kshap1[,1], Kshap1[,2]+Kshap1[,3], Kshap1[,4]))

#         none  group1+group2      group3
# 1: -0.05576529  0.4626137  0.18465409
# 2: -0.05576529  0.4626137  0.09758738
# 3: -0.05576529  0.4626137 -0.79164041
# 4: -0.05576529 -0.6372376  0.18465409
# 5: -0.05576529 -0.6372376  0.09758738
# 6: -0.05576529 -0.6372376 -0.79164041

# - grouping: (feat1, feat3), (feat2)
head(cbind(Kshap1[,1], Kshap1[,2]+Kshap1[,4], Kshap1[,3]))
#         none     group1+group3  group2
# 1: -0.05576529  0.3035614  0.3437063
# 2: -0.05576529  0.2164947  0.3437063
# 3: -0.05576529 -0.6727331  0.3437063
# 4: -0.05576529  0.3035614 -0.7561449
# 5: -0.05576529  0.2164947 -0.7561449
# 6: -0.05576529 -0.6727331 -0.7561449

# - grouping: (feat1, feat3, feat3)
head(cbind(Kshap1[,1], Kshap1[,2]+Kshap1[,4]+ Kshap1[,3]))
#           none     group1
# 1: -0.05576529  0.6472678
# 2: -0.05576529  0.5602011
# 3: -0.05576529 -0.3290267
# 4: -0.05576529 -0.4525835
# 5: -0.05576529 -0.5396502
# 6: -0.05576529 -1.4288780
...
head(cond_expec_mat2)
# 1 = conditioning on nothing, 2 = conditioning on group 1,
# 3 = conditioning on group 2,  4 = conditioning on group 1 and group 2
...
print(head(Kshap2)) # - grouping: (feat1, feat2), (feat3)
#           none     group1      group2
# 1: -0.05576529  0.4625936  0.18467413
# 2: -0.05576529  0.4625936  0.09760742
# 3: -0.05576529  0.4625936 -0.79162037
# 4: -0.05576529 -0.6372576  0.18467413
# 5: -0.05576529 -0.6372576  0.09760742
# 6: -0.05576529 -0.6372576 -0.79162037

# - grouping: (feat1, feat3), (feat1)
# none     group1     group2
# 1: -0.05576529  0.3035414  0.3437264
# 2: -0.05576529  0.2164747  0.3437264
# 3: -0.05576529 -0.6727531  0.3437264
# 4: -0.05576529  0.3035414 -0.7561249
# 5: -0.05576529  0.2164747 -0.7561249
# 6: -0.05576529 -0.6727531 -0.7561249
...
mean(colSums((abs(Kshap1 - explanation_list[['ctree']]$dt))* joint_prob_dt_list[[1]]$joint_prob)[-1])
# 0.02731627
...
print(head(explanation_list[['ctree']]$dt))
#         none     group1      group2
# 1: 0.02618352  0.3828548  0.18246415
# 2: 0.02618352  0.3828548  0.09539743
# 3: 0.02618352  0.3828548 -0.79383035
# 4: 0.02618352 -0.7169964  0.18246415
# 5: 0.02618352 -0.7169964  0.09539743
# 6: 0.02618352 -0.7169964 -0.79383035

mean(colSums((abs(Kshap2 - explanation_list[['ctree']]$dt))* joint_prob_dt_list[[1]]$joint_prob)[-1])
# 0.8447
