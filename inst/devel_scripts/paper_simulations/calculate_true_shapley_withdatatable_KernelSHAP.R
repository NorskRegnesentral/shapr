...
simulate_data <- function(parameters_list){
  ...
  if(noise == TRUE){
    dt_numeric[, epsilon := rnorm(No_test_obs + No_train_obs, 0, 0.1^2)] #
  ...
  # tmp0 <- predict_model(model, x_test)
  ...
  predict_model.numeric_lm <<- function(x, newdata) {
    ...
    # feat_1_ <- factor(newdata[[1]])
    # feat_2_ <- factor(newdata[[2]])
    # feat_3_ <- factor(newdata[[3]])
    # newdata0 <- data.frame(feat_1_, feat_2_, feat_3_)
    # 
    class(x) <- "lm"
    print("HERE")
    predict(x, newdata0)
  ...
  ##
  # tmp <- predict_model(model, x_test_numeric)
  ...
  for(m in methods){
    if(m == 'empirical_ind'){
      ...
      tm0 <- proc.time()
      explanation_list[['kernelSHAP']] <- explain(
        x_test_numeric,
        approach = "empirical",
        type = "independence",
        ...
        mincriterion = 0.95)
      tm1 <- proc.time()
      print(paste0("Finished estimating Shapley value with kernelSHAP method."), quote = FALSE, right = FALSE)
      print(tm1 - tm0)
      timeit['kernelSHAP'] <- list((tm1 - tm0))
