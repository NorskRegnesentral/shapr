...
dir.create(paste("/nr/project/stat/BigInsight/Projects/Fraud/Subprojects/NAV/Annabelle/results/cont_and_cat_data/", tod_date, sep = ""))
...
k <- 1
for(j in corr){
  parameters_list[[k]] <- list(methods = c("gaussian", "ctree", "kernelSHAP"),
...
for(i in 1:length(parameters_list)){
  ...
  saveRDS(all_methods, file = paste("/nr/project/stat/BigInsight/Projects/Fraud/Subprojects/NAV/Annabelle/results/cont_and_cat_data", tod_date, nm, sep = "/"))
}
