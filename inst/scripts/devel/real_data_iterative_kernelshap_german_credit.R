...
dataset_filename = "GermanCreditDataSet.csv"
dataset_name = "german_credit"
GermanCredit <- read.table(paste0("/nr/project/stat//BigInsight//Projects//Explanations//Counterfactual_kode//Carla_datasets//", dataset_filename),header=TRUE, sep=",")

GermanCredit <- unique(GermanCredit)
GermanCredit[,1] <- as.numeric(as.character(unlist(GermanCredit[,1])))

set.seed(100)
nobs <- dim(GermanCredit)[1]
ind <- sample(x=nobs, size=round(0.75*nobs))

GCTrain <- GermanCredit[ind,]
GCTest  <- GermanCredit[-ind,]

params = list(eta = 0.1, max_depth = 2, lambda = 1, alpha = 1)

set.seed(584)
model <- xgboost(data = as.matrix(GCTrain[,-1]),
                 label = GCTrain[,1],
                 nrounds = 500,
...
library(hmeasure)
pred <- predict(model, newdata = as.matrix(GCTest[, -1]))
results <- HMeasure(unlist(as.vector(GCTest[,1])), pred, threshold=0.15)
results$metrics$AUC
