...
plot_vaeac_eval_crit(explanation_list = explanation_list_unnamed)
...
plot_vaeac_eval_crit(explanation_list = explanation_list_named)

# The function also works if we have only one method, but then one should only look at the method plot
plot_vaeac_eval_crit(explanation_list = list("Paired samp. & large NN" = explanation_paired_sampling_TRUE),
                     plot_type = "method")
...
tmp_fig_criterion = plot_vaeac_eval_crit(
  explanation_list = explanation_list_named,
  plot_type = "criterion")
