...
dt_mem0 <- fread("inst/scripts/memory_test_2023.csv")
...
ggplot(dt_mem[p<16& p>2],aes(x=n_batches_real,y=mem_usage_Mb,col=as.factor(n_explain),linetype=as.factor(n_train)))+
  geom_line()+
  geom_point()+
...
dt_time0 <- fread("inst/scripts/timing_test_2023.csv")
...
dt_time <- dt_time0[,.(time,secs_explain,rep,p,n_train,n_explain,n_batches_real,approach)]


ggplot(dt_time[p<16& p>2],aes(x=n_batches_real,y=secs_explain,col=as.factor(n_explain),linetype=as.factor(n_train)))+
  geom_line()+
  geom_point()+
  facet_wrap(vars(approach,p),scales = "free",labeller = label_both)+
#  scale_y_log10()+
