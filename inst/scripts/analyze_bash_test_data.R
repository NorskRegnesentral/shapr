...
dt_mem0 <- fread("inst/scripts/memory_test_2023_tmp.csv")
...
ggplot(dt_mem[p<14],aes(x=n_batches_real,y=mem_usage_Mb,col=as.factor(n_explain),linetype=as.factor(n_train)))+
  geom_line()+
  geom_point()+
...
dt_time0 <- fread("inst/scripts/timing_test_2023_tmp.csv")
...
dt_time[,secs_explain_singlebatch:=mean(secs_explain_singlebatch,na.rm=T),by=.(p,n_train,n_explain,approach)]
