...
make_dummies_list <- make_dummies(traindata = x_train3, testdata = x_test3)
...
# 6) Test has the same levels as train but not every level in the data set
make_dummies_list <- make_dummies(traindata = x_train5, testdata = x_test5)

# 7) Test has different levels than train
x_train7 <- data.table(x_train_cat)
x_train7$rm <- factor(round(x_train7$rm))

x_test7 <- data.table(x_test_cat)
...
# x_train10 <- cbind(x_train_cat, lstat = x_train$lstat)
x_train10 <- x_train_cat
x_test10 <- cbind(x_test_cat, lstat = x_test_cat$lstat)
# x_test10 <- cbind(x_test_cat)

make_dummies_list <- make_dummies(traindata = x_train10, testdata = x_test10) # NO ERROR???
