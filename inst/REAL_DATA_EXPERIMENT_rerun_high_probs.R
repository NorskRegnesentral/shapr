...
run_these_tests <- which(test_preds > 0.5)
run_list <- split(run_these_tests, ceiling(seq_along(run_these_tests)/11))
...
for(i in 2:length(run_list)){
  current.Xtest <- Xtest[run_list[[i]],]#copy(Xtest[run_list[[run_ind]],])
  ...
  saveRDS(Shapley.approx,paste0(resfolder,"/REAL_DATA_EXPERIMENT_RERUN_SHAPR_",i,"_.rds"))

  print(i)
  gc()
}
saveRDS(run_list,paste0(resfolder,"/REAL_DATA_EXPERIMENT_RERUN_runlist_",i,"_.rds"))
