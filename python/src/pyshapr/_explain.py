...
def _wrap_loss_func(loss_func: Callable) -> tuple[Any, Any]:
    """
    Wrap a Python loss function so it can be called from R during the SAGE computation.
    ...
    ----------
    loss_func : Callable
        A Python callable taking two arguments (the true response and the model prediction) and returning a
        single numeric loss value.
    ...
    @ri.rternalize
    def r_loss(y: Any, pred: Any) -> FloatVector:
        return FloatVector([float(loss_func(np.asarray(y), np.asarray(pred)))])
...
def explain(
    ...
    **kwargs: Further arguments passed to specific approaches.
    ...
    r_group = (
        NULL
        if group is None
        else ListVector({key: StrVector(value) for key, value in group.items()})
    )
    ...
    r_causal_ordering = (
        ...
        else ListVector({key: StrVector(value) for key, value in causal_ordering.items()})
    )
    ...
    predict_model = get_predict_model(
        x_test=x_train.head(2), predict_model=predict_model, model=model
    )

    rinternal.rx2["timing_list"].rx2["test_prediction"] = base.Sys_time()

    rinternal = additional_regression_setup(rinternal, model, predict_model, x_train, x_explain)

    # Not called for approach %in% c("regression_surrogate","vaeac")
    rinternal = shapr.setup_approach(
        internal=rinternal
    )  # model and predict_model are not supported in Python
    ...
    while not converged:
        ...
        rinternal.rx2["timing_list"] = ro.ListVector(
            {**dict(rinternal.rx2["timing_list"].items()), "postprocess_res": base.Sys_time()}
        )

        # Add the current timing_list to the iter_timing_list
        rinternal.rx2["iter_timing_list"] = ro.ListVector(
            {
                **dict(rinternal.rx2["iter_timing_list"].items()),
                f"element_{iter}": rinternal.rx2["timing_list"],
            }
        )

        iter += 1

    rinternal.rx2["main_timing_list"] = ro.ListVector(
        {**dict(rinternal.rx2["main_timing_list"].items()), "main_computation": base.Sys_time()}
    )
    ...
    rinternal.rx2["main_timing_list"] = ro.ListVector(
        {**dict(rinternal.rx2["main_timing_list"].items()), "finalize_explanation": base.Sys_time()}
    )
...
def compute_vS(rinternal: Any, model: Any, predict_model: Callable) -> Any:
    ...
    stats.rnorm(
        1
    )  # Perform a single sample to forward the RNG state one step. This is done to ensurie consistency with
    ...
    for i, S in enumerate(S_batch):
        vS_list.rx2[i + 1] = batch_compute_vS(
            S=S, rinternal=rinternal, model=model, predict_model=predict_model
        )
