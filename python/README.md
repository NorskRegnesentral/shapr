...
<!-- BEGIN GENERATED SHAPR COMPATIBILITY -->
...
<!-- END GENERATED SHAPR COMPATIBILITY -->

Until the next CRAN release of `shapr`, install the development version from GitHub using:

```bash
Rscript -e 'pak::pak("NorskRegnesentral/shapr")'
```

When `explain()` is first called, `pyshapr` loads the R backend and checks its version. For a
version in the limited-support range, it emits a `ShaprVersionWarning` that identifies the
unavailable functionality while allowing supported SHAP functionality to run. Requesting SAGE
with a `shapr` version that does not expose the SAGE API raises an error because such a version
can silently return local SHAP values instead of SAGE values. Importing `pyshapr` or `explain`
remains lightweight and does not initialize R.
