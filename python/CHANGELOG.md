...
All notable changes to the Python `pyshapr` library will be documented in this file.
...
### Fixed
- Constrained `numpy<2.5` to keep `pip install pyshapr` resolvable on Python 3.12 and 3.13. No `numba`
  release yet supports numpy 2.5, so an unconstrained install (via the transitive `shap` -> `numba`
  dependency) backtracked to an ancient, incompatible `numba` that fails to build. The cap may be lifted
  once `numba` supports numpy 2.5.
...
- **NEW**: `ShaprSummary` class for enhanced summary functionality
  - Dedicated class for handling summary results with improved display and access methods
  - Support for tab-completion in IPython/Jupyter environments
- Adjusted the unit tests to cover the new `ShaprSummary` class functionality

### Changed
- Update `class_exploration.py` example to demonstrate the new `ShaprSummary` class
