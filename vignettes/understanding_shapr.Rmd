...
This will, however, throw a warning that all feature consistency checking towards the model is disabled.
...
Below we exemplify both a set of full support versions of these functions and a minimal version also skipping the
`get_model_specs` function. 
We do this for the `gbm` model class from the pacakge `gbm` package, fitted to the same Boston data set as used above.
