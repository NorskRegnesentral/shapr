...
We can use mixed (i.e continuous, categorical, ordinal) data with ctree. Use ctree with categorical data in the following manner. Note that two additional steps are required when using categorical data with xgboost: make_dummies() and apply_dummies(). The function make_dummies() converts the factors to one-hot encoded variables (this is necessary for xgboost). We have to add dummyfunc to the model (see model_cat$dummylist) below so that shapr() doesn't throw an error and say that the columns of x_train are different then the features used to train the model (i.e model_cat).

```{r}
x_var <- c("lstat", "chas", "rad", "indus")
y_var <- "medv"

data <- Boston # so we don't change the cont variables to factor variables permanently

# convert to factors
data$rad = as.factor(data$rad)
data$chas = as.factor(data$chas)

x_train_cat <- data[-1:-6, x_var]
y_train_cat <- data[-1:-6, y_var]
x_test_cat <- data[1:6, x_var]

# convert factors to one-hot encoded variables for xgboost
dummyfunc <- make_dummies(data = rbind(x_train_cat, x_test_cat))

x_train_dummies <- apply_dummies(obj = dummyfunc, newdata = x_train_cat)
...
# an important step so that shapr() doesn't throw an error
model_cat$dummylist <- dummyfunc
...
Can also check that the correct test observations are being explained: 

```{r}

x_test_dummies <- apply_dummies(obj = dummyfunc, newdata = x_test_cat)

print(predict(model_cat, x_test_dummies))

print(explanation_ctree$p)
