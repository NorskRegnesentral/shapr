...
```{r}
# The function will set default names and give a message to the user
plots_unnamed <- make_MSEv_eval_crit_plots(explanation_list_unnamed)

# The function use the provided names
plots_named <- make_MSEv_eval_crit_plots(explanation_list_named)

# See the types of plots produced
names(plots_named)
```
The main plot if interest is the `bar_plot_MSEv`, which displays the $\operatorname{MSE}_{v}$ evaluation criterion for
each method averaged over both the coalitions and explicands. However, we can also look at the other plots where
we have only averaged over the explicands or the coalitions (both as bar and line plots).

```{r}
# The main plot of the overall MSEv averaged over both the coalitions and explicands
plots_named$MSEv_bar

# The MSEv averaged over only the coalitions
plots_named$MSEv_explicand_bar

# The MSEv averaged over only the explicands
plots_named$MSEv_combination_bar
```
...
```{r}
plots_named_v2 <- make_MSEv_eval_crit_plots(
  explanation_list_named,
  index_combinations = c(1:3, 9, 11),
...
plots_named_v2$MSEv_combination_bar
plots_named_v2$MSEv_explicand_line_point

# Plot created without altering the plotting parameters
plots_named$MSEv_explicand_line_point
```
