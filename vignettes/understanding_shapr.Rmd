...
When using `prepare_kshap`, the full sum is by default used in the Shapley formula. A sampling approach may be used by setting `exact=FALSE`, and the number of samples used must then be specified by the argument `noSamp`. When computing the kernel SHAP values by `compute_kshap`, the maximum number of samples to use in the Monte Carlo integration for every conditional expectation is controlled by the argument `noSamp_MC`, by deafult set to be 1000. As seen in the code above, the argument `pred_zero` for the prediction value of the unseen data is required, and typically this is equal to the mean of the response. If the empirical method is used, the settings to use can be specified through the argument `empirical_settings`. If the Gaussian approach is used, we may specify the mean vector and covariance matrix of the data generating distribution by the arguments `mu` and `Sigma`. If not specified, they are estimated from the data.  
