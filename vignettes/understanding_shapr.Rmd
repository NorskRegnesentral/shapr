...
If all features are categorical, the package implements a `categorical` method. 
...
The following example shows how a simple `xgboost` model is trained using the `airquality` data set, 
...
The Gaussian, Gaussian copula, ctree, categorical or independence approaches can be used instead by setting the argument `approach` to either 
`"gaussian"`, `"copula"`, `"ctree"`, `"categorical"` or `"independence"`.
...
# Plot the resulting explanations for observations 1 and 6, excluding 
# the no-covariate effect
plot(explanation_cat_method, plot_phi0 = FALSE, index_x_explain = c(1, 6))
```
...
that approach, like a vector of fixed $\sigma$ values can be specified through the argument `empirical.fixed_sigma_vec`. See `?explain` 
for more information. If `approach = "gaussian"`, you may specify the mean vector and covariance matrix of the data 
generating distribution by the arguments `gaussian.mu` and `gaussian.cov_mat`. If not specified, they are estimated from the training data.  
...
In addition to letting the user select one of the five aforementioned approaches for estimating the conditional 
distribution of the data (i.e. `approach` equals either [`"gaussian"`](#gaussian), [`"copula"`](#copula),
[`"empirical"`](#empirical), [`"ctree"`](#ctree) or [`"categorical"`](#categorical)), the package allows the user to combine the five approaches. To simplify the usage, the flexibility is restricted such that the same approach is used when conditioning on the same number of features. 
...
`empirical.eta = 1` when using `explain`, we can approximate the original method arbitrarily 
