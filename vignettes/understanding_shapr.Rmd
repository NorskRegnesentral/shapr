...
With time series data, the timeseries approach can be used. Instead of deriving a Shapley value for each time point, we derive a Shapley value for groups of consecutive time points, indicated using the `group` attribute. Other optional parameters include `timeseries.fixed_sigma_vec` and `timeseries.bounds` (a vector indicating upper and lower bounds of the time series if necessary).  
...
group_timeseries <- list(Q1 = paste0("X", 1:10),
                         Q2 = paste0("X", 11:20),
                         Q3 = paste0("X", 21:30),
                         Q4 = paste0("X", 31:40))


lm_ts_formula = as.formula(paste0("X41", "~ ", paste0("X", 1:40, collapse = " + ")))
model_lm_timeseries = lm(lm_ts_formula, data_ts)

x_explain_timeseries = data_ts[1:2, 1:40]
x_train_timeseries = data_ts[99:100, 1:40]

p0_timeseries <- mean(data_ts[, 'X41'])

explanation_cat_method <- explain(
    model = model_lm_timeseries,
