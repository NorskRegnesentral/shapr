...
The `t` value controls the convergence tolerance, defaults to 0.02, and can be set through the `adaptive_args$convergence_tolerance` argument, see [Adaptive estimation](#adaptive) for more details.
...
Specifics related to the adaptive process and convergence criterion are set through `adaptive_args` argument.
...
ex <- explain(
  ...
  prediction_zero = p0,
  adaptive = TRUE,
  adaptive_args = list(convergence_tolerance = 0.1)
)
#> Note: Feature classes extracted from the model contains NA.
#> Assuming feature classes from the data are correct.
#> Success with message:
...
`adaptive_args$max_batch_size` (defaults to 10) and
`adaptive_args$min_n_batches` (defaults to 10).
...
The latter is found at `SHAPR_OBJ$internal$parameters$adaptive_args$saving_path`, defaults to a temporary folder,
...
ex_further <- explain(
  ...
  prediction_zero = p0,
  max_n_coalitions = 25,
  adaptive_args = list(convergence_tolerance = 0.005), # Decrease the convergence threshold
...
print(ex_further$internal$parameters$adaptive_args$saving_path)
...
ex_even_further <- explain(
  ...
  prev_shapr_object = ex_further$internal$parameters$adaptive_args$saving_path
)
#> Note: Feature classes extracted from the model contains NA.
#> Assuming feature classes from the data are correct.
#> Success with message:
