...
Conditional inference trees (@hothorn2006unbiased) of tree fitting procedure that can handle mixed (i.e continuous, discrete, ordinal, categorical) features and both univariate and multivariate responses. We can use conditional inference trees (ctree) to model the conditional distribution, $p(\boldsymbol{x}_{\bar{\mathcal{S}}}|\boldsymbol{x}_{\mathcal{S}}^*)$, found in the Shapley methodology 

We implement this method (called the *ctree* method) using the *party* and *partykit* packages (@partykit_package). 
