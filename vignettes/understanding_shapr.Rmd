...
`shapr` exports a specific function to explain forecasts created by
time series models, called `explain_forecast`. The main difference
compared to `explain` is that the data is supplied as a set of time series,
and indices denoting different points in time select the train and explain
parts of the data. See `?explain_forecast` for more information.
...
data an arima model of order (2, 0, 0) is fitted.
...
This means that one forecast of $t = 500:502$ and $t = 501:503$ will be explained.
...
In the same way, `train_idx` denotes the points in time which the different
features of the model can be replaced by. Note that if a model requires two
lags, the smallest value of `train_idx` must also be 2, because at time $t = 1$
there was only a single observation available.

Since the data is stationary, the mean of the data is used as prediction zero,
this can however be chosen differently depending on the data and application.

For a multivariate model such as a VAR, it may be of more interest to explain
...
To give a more hands on example, say that we have an AR(2) model which describes the change over time of the variable
...
```
To explain the forecasted value as a function of the two autoregressive terms in the model, we use `explain_forecast`.
First, we pass the model and the data as `model` and `y`. Then, we let `shapr` know which time indices to use as
training data through the argument `train_idx`. We use `2:152`, meaning that we skip the first index, as it is not
possible to make a forecast with an AR(2) model with only one observation. Letting the training indices go up until 152
means that every point in time except the first and last will be used as training data.

The last index, 153 is passed as the argument `explain_idx`, which means that we want to explain a forecast made from
time point 153 in the data. Next, we set `explain_y_lags` to 2, requesting an explanation of autoregressive lag 1 and 2.
...
use an ARIMA(2,0,0) model with `Wind` as an exogenous regressor. The exogenous regressor must be available for the
predicted time points, why the model is just fit on the 151 first observations, leaving two observations of `Wind` to be
