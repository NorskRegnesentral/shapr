...
plot_MSEv_eval_crit(explanation_list_named)$MSEv_bar +
...
Can look at the training and validation error for the trained `vaeac` model and see that `vaeac.epochs = 10` is likely to few epochs as it still seems like the `vaeac` model is learning.
```{r}
# Look at the training and validation errors.
make_vaeac_training_evaluation_plots(list("Vaeac 3 epochs" = explanation_vaeac),
                                     plot_figures = FALSE,
                                     return_figures = TRUE)$figure_facet_for_each_method
```
...
allow the `vaeac` model to not improve its validation score. That is, if `vaeac.epochs_early_stopping = 3`,
then `vaeac` will stop the training procedure if there has been no improvement in the validation score 
for `3` consecutive epochs, of if `vaeac.epochs` is reached. Note that if using early stopping and progress
updates simultaneously, then the estimated timer remaining will obviously be incorrect if early stopping 
is applied.


```{r}
explanation_vaeac_early_stopping <- explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = "vaeac",
...
# Look at the training and validation errors.
make_vaeac_training_evaluation_plots(list("Vaeac 3 epochs" = explanation_vaeac,
                                          "Vaeac early stopping" = explanation_vaeac_early_stopping),
                                     plot_figures = FALSE,
                                     return_figures = TRUE)$figure_facet_for_each_method
```

Can also compare the $\operatorname{MSE}_{v}$ evaluation scores.

```{r}
plot_MSEv_eval_crit(list("Vaeac 3 epochs" = explanation_vaeac,
                         "Vaeac early stopping" = explanation_vaeac_early_stopping),
                    plot_type = c("overall"))$MSEv_bar
```
...
Note: if `approach = vaeac`, then `progressr` will display two progress bars.
The first progress bar displays the training progress of the `vaeac` model, while
the second displays the progress of estimating the contribution functions.
Only the second progress bar are displayed for the other approaches. Furthermore,
if `vaeac.verbose = TRUE` in the call to `explain`, then double sets of progress
bars will be shown.
