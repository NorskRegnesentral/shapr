...
Conditional inference trees (@hothorn2006unbiasedis) of tree fitting procedure that can handle mixed (i.e continuous, discrete, ordinal, categorical) features and both univariate and multivariate responses. We can use conditional inference trees (ctree) to model the conditional distribution, $p(\boldsymbol{x}_{\bar{\mathcal{S}}}|\boldsymbol{x}_{\mathcal{S}}^*)$, found in the Shapley methodology 

We implement this method (called the `ctree' method) using the \verb|party| and \verb|partykit| packages (@partykit_package). 
...
`"gaussian"` or `"copula"`. The ctree package can be used by setting the argument `approach` to `"ctree"`. 
...
We can specify parameters used to build the conditional inference trees using the following manner. Default values based on @hothorn2006unbiasedis.  
...
# Default parameters (based on (Hothorn, 2006)):
# mincriterion = 0.95
# minsplit = 20
# minbucket = 7
...
We can also specify multiple different mincriterion parameters to use when conditioning on different numbers of features. In this case, a vector of length = number of cols must be provided. 

```{r}
# Use the conditional inference tree approach
# Specify a vector of mincriterions instead of just one
# In this case, when conditioning on 1 or 2 features, use mincriterion = 0.95
# When conditioning on 3 or 4 features, use mincriterion = 0.25
