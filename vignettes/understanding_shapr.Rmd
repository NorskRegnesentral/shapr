...
Below we a basic example of timing and memory consumption with differnt 
`n_batches`:

```{r}

Rprof_tab <- NULL
# default n_batches (NULL), corresponding to 10 for this example
Rprof(Rprof0  <- tempfile(), memory.profiling=TRUE,interval=0.001)
  explanation <- explain(
    model = model,
    x_explain = x_explain_many,
    x_train = x_train,
...
summary_Rprof <- summaryRprof(Rprof0, memory="both")
memory <- max(summary_Rprof$by.total$mem.total)
timing <- summary_Rprof$by.total$total.time[1] # Corresponding explain()
Rprof_tab <- rbind(Rprof_tab,
                   c(n_batches = "NULL (10)",
                     memory = memory,
                     timing = timing)
                   )
...
TODO: ADD EXAMPLE WITH N_BATCHES HERE, PRINTING THE COMPUTATION TIME, and writing out the memory consumed in this process manually (or with some base r package.)
...
at least not with many parallel sessions.
Parallelizatiob also increases the memory consumption proportionally, so you want
to limit the number of parallel sessions for that reason too.
In a future version of `shapr` we will provide experienced based automatic selection
of the number of parallel sessions. In the meanwhile, this is all left to the user.
TODO: ADD EXAMPLE WITH PARALLELIZATION HERE.
