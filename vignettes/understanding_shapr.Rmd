...
model_mixed <- lm(medv ~ lstat + rm + dis + indus, data = xy_train_cat)

# Prepare the data for explanation
explainer <- shapr(x_train_cat, model_mixed)
...
group_list <- list(A = c("lstat","rm"),
              B = c("dis","indus"))

# Set up the explainer using the groups
explainer_group <- shapr(x_train,model,group = group_list)
...
plot(explanation_group, plot_phi0 = T, index_x_test = c(1, 6))
