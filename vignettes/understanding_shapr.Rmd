...
```{r}
plot(explanation, plot_type = "scatter", scatter_hist = TRUE)
```
...
plot(explanation_cat, bar_plot_phi0 = FALSE, index_x_explain = c(1, 6))
```
...
If __all__ features are categorical, the categorical approach is used as follows:
...
# convert _all_ features to factors
...
x_var_cat <- c("Solar.R_factor", "Wind_factor", "Temp_factor", "Month_factor")
...
```{r}
Rprof("Rprof.out", memory.profiling=TRUE,interval=0.001)
  explanation <- explain(
 model = model,
 x_explain = x_explain,
 x_train = x_train,
