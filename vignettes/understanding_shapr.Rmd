...
We allow for some alterations when computing the $\operatorname{MSE}_{v}$ evaluation criterion.
We can exclude the empty ($\mathcal{S} = \emptyset$) and full ($\mathcal{S} = \mathcal{M}$) coalitions from the 
computations by setting `MSEv_skip_empty_full_comb = TRUE`. This does not influence the ranking of the methods
as the methods are not used to compute the contribution function for the empty and full coalitions.
Furthermore, when `MSEv_uniform_comb_weights = TRUE`, we weight the combinations/coalitions uniformly. 
This is done above in the $\operatorname{MSE}_{v}$ equation when we divide by $N_\mathcal{S}$. However,
if `MSEv_uniform_comb_weights = FALSE`, then we weight the combinations using the corresponding Shapley kernel
weights (or the sampling frequencies when sampling of combinations is used). 
The default is to include the empty and full combinations and to weight all combinations uniformly.
...
```{r}
# These are the default values in `explain()`
...
to only plot certain combinations and/or test observations.
See the documentation of the `make_MSEv_eval_crit_plots()` function for several examples.

```{r, echo=FALSE, message=FALSE, results='hide'}
bar_text_n_decimals <- 1
MSEv_plot <- make_MSEv_eval_crit_plots(explanation_list_named, N_sd_error_bars = 1)
MSEv_plot +
  ...
  ggplot2::geom_text(ggplot2::aes(label = sprintf(paste("%.", sprintf("%d", bar_text_n_decimals), "f", sep = ""),
                     ...
                     vjust = -0.35, # This number might need altering for diffent plots sizes
                     hjust = 1.15, # This number might need altering for diffent plots sizes
                     color = "black",
