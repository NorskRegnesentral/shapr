...
vaeac.save_every_nth_epoch <- 3 # Save every 3th epoch + best and last epoch
...
vaeac.cuda <- FALSE # We run everything on CPU, but all code below works on GPU to
...
expl_batches_combinations_2 <- explain(
  ...
  vaeac.epochs = 3,
  verbose = 2,
  vaeac.extra_parameters = list(vaeac.cuda = vaeac.cuda)
)
#> Note: Feature classes extracted from the model contains NA.
#> Assuming feature classes from the data are correct.
#> Starting 'setup_approach.vaeac'.
#> Training a vaeac model with the provided parameters from scratch on the CPU.
...
#> Done with 'setup_approach.vaeac'.
#> Working on batch 1 of 1 in `prepare_data.vaeac()`.
...
#> Paired  0.060336        0.044753            7.2318    0.30695           0.0045791
#> Regular 0.058398        0.042936            6.5810    0.30968           0.0047970
```
...
#> Done with 'setup_approach.vaeac'.
#> Working on batch 1 of 5 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
#> Working on batch 2 of 5 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
#> Working on batch 3 of 5 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
#> Working on batch 4 of 5 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
#> Working on batch 5 of 5 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
...
expl_early_stopping <- explain(
  ...
  vaeac.n_vaeacs_initialize = 2,
  vaeac.extra_parameters = list(
    vaeac.epochs_early_stopping = 2, # Low value here for faster building of the vignette
    vaeac.cuda = vaeac.cuda
  )
)
#> Note: Feature classes extracted from the model contains NA.
...
#> Done with 'setup_approach.vaeac'.
#> Working on batch 1 of 2 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
#> Working on batch 2 of 2 in `prepare_data.vaeac()`.
#> Preprocessing the explicands.
#> Generating the MC samples.
#> Postprocessing the Monte Carlo samples.
...
there are often no benefit of using a GPU. However, this depends on what kind of CPU and GPU the user has access to.
...
diag(sigma) <- 1
x_train <- mvtnorm::rmvnorm(n, mean, sigma)
x_explain <- mvtnorm::rmvnorm(n, mean, sigma)
...
#> Cuda/GPU is not available (`shapr` uses CPU instead).TRUE
...
rbind("Vaeac CPU" = explanation_cpu$timing$timing_secs, "Vaeac GPU" = explanation_cuda$timing$timing_secs)
#>              setup test_prediction setup_computation compute_vS shapley_computation
#> Vaeac CPU 0.085639        0.045625            61.775     76.989            0.039412
#> Vaeac GPU 0.067961        0.046196            64.400     76.386            0.039012
rbind("Vaeac CPU" = time_cpu, "Vaeac GPU" = time_cuda)
#>           user.self sys.self elapsed user.child sys.child
#> Vaeac CPU    687.89   941.38  138.94          0         0
#> Vaeac GPU    705.69   985.45  140.94          0         0
...
![](figure_vaeac/unnamed-chunk-2-1.png)

```r

# Almost identical MSEv values
plot_MSEv_eval_crit(list("Vaeac CPU" = explanation_cpu, "Vaeac GPU" = explanation_cuda))
```

![](figure_vaeac/unnamed-chunk-2-2.png)

```r

# Can compare the Shapley values and see that we get comparable values
plot_SV_several_approaches(
  list("Vaeac CPU" = explanation_cpu, "Vaeac GPU" = explanation_cuda),
...
![](figure_vaeac/unnamed-chunk-2-3.png)
