...
vaeac_plot_evaluation_criteria(explanation_list)
...
For more visual information, we can use the `progressr` package too. This can help us see the progress of the training 
step for the final `vaeac` model. Note that one can set `verbose = 0` to not get any sticky messages from the `vaeac`
...
vaeac_plot_evaluation_criteria(list("Original" = expl_litle_training))
...
vaeac_plot_evaluation_criteria(list("Vaeac early stopping" = expl_early_stopping))
```
...
# Specifying the phi_0, i.e. the expected prediction without any features
p0 <- mean(data_train_cat[, get(y_var)])
...
expl_vaeac <- explain(
  ...
  vaeac.latent_dim = 4,
  vaeac.extra_parameters = list(
    vaeac.skip_connection_layer = FALSE,
...
vaeac_plot_evaluation_criteria(list("Simple vaeac" = expl_vaeac))
...
We now make a more complex vaeac model and train it for longer to obtain better results (GJØR VI DET? ALTSÅ BEDRE RESULTATER?)
