...
library(xgboost)
library(data.table)
#> data.table 1.15.2 using 10 threads (see ?getDTthreads).  Latest news: r-datatable.com
...
#> Error in plot_SV_several_approaches(list(Original = explanation, `Other combi.` = expl_batches_combinations)): The object/objects 'Other combi.' in `explanation_list` has/have a different `x_explain` than 'Original'. Cannot compare them.
...
#> Paired  0.21440387      0.02784705          3.935513  0.1883459         0.003329039
#> Regular 0.04577899      0.03394413          3.906748  0.2306199         0.003444910
```



## Progressr {#progress_bar}
As discussed in the main vignette, the `shapr` package provides two ways for recieving information about the progress
...
#> Error in handlers("void"): could not find function "handlers"
...
#> Error in MSEv_check_explanation_list(explanation_list): The object/objects 'Vaeac early stopping more epochs' in `explanation_list` has/have a different `x_explain` than 'Vaeac early stopping'. Cannot compare them.
# We see that the Shapley values have changed, but only slightly
plot_SV_several_approaches(list(
  "Vaeac early stopping" = expl_early_stopping,
  "Vaeac early stopping more epochs" = expl_early_stopping_train_more
))
#> Error in plot_SV_several_approaches(list(`Vaeac early stopping` = expl_early_stopping, : The object/objects 'Vaeac early stopping more epochs' in `explanation_list` has/have a different `x_explain` than 'Vaeac early stopping'. Cannot compare them.
```
...
there are often no benefit of using a GPU. However, this depends on what kind of CPU and GPU the user has access to. As our CPU, we use a 13th Gen Intel(R) Core(TM) i7-13700H 2.40 GHz and 32.0 GB 4800 MHz RAM. While for the GPU, we use a NVIDIA GeForce RTX 4050 Laptop with 6 GB dedicated memmory and 16 GB shared. Furthermore, the times depends on several factors, e.g., the number of training observations, explicands, features, and
batches.
