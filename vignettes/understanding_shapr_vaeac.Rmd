...
to remedy poorly initiated model parameter values (`vaeac.num_different_vaeac_initiate`), and
...
all.equal(explanation$internal$output$dt_samp_for_vS,
          explanation_pretrained_vaeac_model$internal$output$dt_samp_for_vS)
```
...
all.equal(explanation$internal$output$dt_samp_for_vS,
          explanation_pretrained_vaeac_model_path$internal$output$dt_samp_for_vS)
```
...
make_bar_plot_several_explanation_objects(
  list("Original" = explanation,
       "Other combi." = explanation_batches_combinations))
...
explanation_misplaced_parameters = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
  ...
  vaeac.extra_parameters = list(
    ...
    vaeac.save_vaeac_every_nth_epoch = vaeac.save_vaeac_every_nth_epoch, # Correct place
    what_is_this_parameter = 4   # Unknown parameter
    )
  )

# Look at the used parameters that was misplaced
explanation_misplaced_parameters$internal$parameters[c("vaeac.epochs", "vaeac.verbose", "vaeac.depth")]
```
...
make_vaeac_training_evaluation_plots(list("Regular samp." = explanation_paired_sampling_FALSE,
                                          "Paired samp." = explanation_paired_sampling_TRUE))

plot_MSEv_eval_crit(list("Regular samp." = explanation_paired_sampling_FALSE,
                         "Paired samp." = explanation_paired_sampling_TRUE))$MSEv_bar
```
...
rbind("time_paired_sampling" = explanation_paired_sampling_TRUE$timing$timing_secs,
      "time_regular_sampling" = explanation_paired_sampling_FALSE$timing$timing_secs)
```
...
explanation_without_messages = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
...
handlers("void") # To silence all progressr updates
explanation_with_messages = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
...
all.equal(explanation_without_messages$shapley_values, explanation_with_messages$shapley_values)
```
...
# ))
with_progress({
explanation_with_progressr = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
...
all.equal(explanation_without_messages$shapley_values, explanation_with_progressr$shapley_values)
```
...
handlers("progress")
with_progress({
explanation_with_progressr_and_verbose = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
...
all.equal(explanation_without_messages$shapley_values, explanation_with_progressr_and_verbose$shapley_values)
```
...
explanation_litle_training = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
...
vaeac_ggpairs_plot_imputed_and_true_data_shapr(explanation = explanation_litle_training,
                                               which_vaeac_model = "best",
                                               true_data = x_train)

# Look at the training and validation errors. Not happy and want to train more.
make_vaeac_training_evaluation_plots(explanation_litle_training)

# Can look at the saved models, training/validation errors, and the training time
explanation_litle_training$internal$parameters$vaeac$models
explanation_litle_training$internal$parameters$vaeac$results
explanation_litle_training$internal$parameters$vaeac$training_time

# Make a copy of the explanation object which we are to train further.
explanation_train_more = explanation_litle_training

# Continue to train the vaeac model some more epochs
explanation_train_more$internal$parameters$vaeac =
  vaeac_continue_train_model(explanation = explanation_train_more,
                             epochs_new = 5,
                             training_data = x_train,
                             verbose = TRUE)

# Can see variables have been updated
explanation_train_more$internal$parameters$vaeac$training_time
explanation_train_more$internal$parameters$vaeac$models
explanation_train_more$internal$parameters$vaeac$results

# Look at the training and validation errors. Still not sure
# Note that the lines are obviously overlapping
make_vaeac_training_evaluation_plots(list("Original" = explanation_litle_training,
                                          "More epochs" = explanation_train_more))

# Use train more vaeac model to compute Shapley values again.
explanation_train_more_vaeac_model = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
  ...
  seed = seed,
  vaeac.extra_parameters = list(
    vaeac.pretrained_vaeac_model = explanation_train_more$internal$parameters$vaeac)
  )

# Train even more
explanation_train_even_more = explanation_train_more

# Continue to train the vaeac model some more epochs
explanation_train_even_more$internal$parameters$vaeac =
  vaeac_continue_train_model(explanation = explanation_train_even_more,
                             epochs_new = 10,
                             training_data = x_train,
                             verbose = TRUE)

# Variables have been updated once again
explanation_train_even_more$internal$parameters$vaeac$training_time
explanation_train_even_more$internal$parameters$vaeac$models
explanation_train_even_more$internal$parameters$vaeac$results

# Look at the training and validation errors.
make_vaeac_training_evaluation_plots(list("Original" = explanation_litle_training,
                                          "More epochs" = explanation_train_more,
                                          "Even more epochs" = explanation_train_even_more))

# Can also see how well vaeac generates data from the full joint distribution
vaeac_ggpairs_plot_imputed_and_true_data_shapr(explanation = explanation_train_even_more,
                                               which_vaeac_model = "best",
                                               true_data = x_train)
  
# Use train even more vaeac model to compute Shapley values again.
explanation_train_even_more_vaeac_model = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
  ...
  seed = seed,
  vaeac.extra_parameters = list(
    vaeac.pretrained_vaeac_model = explanation_train_even_more$internal$parameters$vaeac)
  )

# We see that the Shapley values have changed
make_bar_plot_several_explanation_objects(
  explanation_list = list(
    "Few epochs" = explanation_litle_training,
    "More epochs" = explanation_train_more_vaeac_model,
    "Even more epochs" = explanation_train_even_more_vaeac_model))
...
plot_MSEv_eval_crit(list("Few epochs" = explanation_litle_training,
       "More epochs" = explanation_train_more_vaeac_model,
       "Even more epochs" = explanation_train_even_more_vaeac_model))$MSEv_bar
```
...
explanation_early_stopping = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
...
make_vaeac_training_evaluation_plots(list("Vaeac early stopping" = explanation_early_stopping))
```

However, we can train it further for a fixed amount of epochs if desired.
```{r early stopping 2, cache = TRUE}
# Make a copy of the explanation object which we are to train further.
explanation_early_stopping_train_more = explanation_early_stopping

# Continue to train the vaeac model some more epochs
explanation_early_stopping_train_more$internal$parameters$vaeac =
  vaeac_continue_train_model(explanation = explanation_early_stopping_train_more,
                             epochs_new = 15,
                             training_data = x_train,
                             verbose = TRUE)
...
# Look at the training and validation errors. 
make_vaeac_training_evaluation_plots(list("Vaeac early stopping" = explanation_early_stopping,
                                          "Vaeac early stopping more epochs" = explanation_early_stopping_train_more))
```
...
explanation_early_stopping_train_more = explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = approach,
  ...
  seed = seed,
  vaeac.extra_parameters = list(
    vaeac.pretrained_vaeac_model = explanation_early_stopping_train_more$internal$parameters$vaeac)
  )
```
...
# We can compare them
plot_MSEv_eval_crit(
  list("Vaeac early stopping" = explanation_early_stopping,
       "Vaeac early stopping more epochs" = explanation_early_stopping_train_more))$MSEv_bar

# We see that the Shapley values have changed
make_bar_plot_several_explanation_objects(
  explanation_list = list("Vaeac early stopping" = explanation_early_stopping,
                          "Vaeac early stopping more epochs" = explanation_early_stopping_train_more),
...
```{r vaeac mixed data, cache = TRUE}
  ...
  make_vaeac_training_evaluation_plots(list("Simple vaeac" = explanation_vaeac))
...
```{r complex vaeac, cache = TRUE}
 # Here we fit a more complex vaeac method
  ...
  make_vaeac_training_evaluation_plots(list("Vaeac w.o. skip-con." = explanation_vaeac,
                                            "Vaeac w. skip-con." = explanation_vaeac_complex))
...
```{r compare for mixed data, cache = TRUE}
  # Can see that the extra training has decreased the MSE_Frye evaluation criterion
  plot_MSEv_eval_crit(list("Vaeac w.o. skip-con." = explanation_vaeac,
                                            "Vaeac w. skip-con." = explanation_vaeac_complex,
                                            "Ctree" = explanation_ctree))$MSEv_bar

  
  # Can compare the Shapley values
  make_bar_plot_several_explanation_objects(list("Vaeac w.o. skip-con." = explanation_vaeac,
                                            "Vaeac w. skip-con." = explanation_vaeac_complex,
