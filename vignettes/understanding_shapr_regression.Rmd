...
however, we have to add a preprocessing step where we encode categorical features for some of the regresssion models.
...
We skip the printout for the second version due to the length of the printouts.
...
explanation_list$sep_rf_cv <- explain(
  ...
  regression_tune_values =
    function(x) {
      return(dials::grid_regular(dials::mtry(c(1, ncol(x))), dials::trees(c(50, 750)), levels = 3))
    },
  regression_vfold_cv_para = list(v = 5)
)
```
...
explanation_list$sep_xgboost_cv_2_par <- explain(
  ...
  regression_model = parsnip::boost_tree(
    trees = hardhat::tune(), tree_depth = hardhat::tune(), engine = "xgboost", mode = "regression"
  ),
...
# We can also order them to more easily look at the order of the methods
...
Here we focus on the 5 best methods
...
explanation_list_mixed$sep_tree_cv <- explain(
  ...
  approach = "regression_separate",
  regression_model = parsnip::decision_tree(
    tree_depth = hardhat::tune(), cost_complexity = hardhat::tune(), engine = "rpart", mode = "regression"
  ),
  regression_tune_values =
    expand.grid(tree_depth = c(1, 3, 5), cost_complexity = c(0.001, 0.01, 0.1)),
  regression_vfold_cv_para = list(v = 5)
...
explanation_list_mixed$sur_tree_cv <- explain(
  ...
  approach = "regression_surrogate",
  regression_model = parsnip::decision_tree(
    tree_depth = hardhat::tune(), cost_complexity = hardhat::tune(), engine = "xgboost", mode = "regression"
  ),
  regression_tune_values =
    expand.grid(tree_depth = c(1, 3, 5), cost_complexity = c(0.001, 0.01, 0.1)),
  regression_vfold_cv_para = list(v = 5)
...
# Can also focus on the best 5 (+ ctree)
