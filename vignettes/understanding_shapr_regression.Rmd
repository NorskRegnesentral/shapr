...
set.seed(123) # Set seed for reproducibility in the example in the next code chunk
...
specify the model using the `tidymodels` package to obtain the same model as we can see in the example below. Note
that `shapr` supports any fitted `tidymodels` models via the `workflows` procedure outlined below.
...
all.equal(explanation_list$MC_empirical_tidymodels$shapley_values,
explanation_list$MC_empirical$shapley values)
```
