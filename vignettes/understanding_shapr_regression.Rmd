...
# Note: Needs to be loaded after shapr, otherwise we need to use the 'shapr::' prefix when calling explain()
library(tidymodels)
...
#> Error in make.unique(names, sep = "_"): 'names' must be a character vector
```
...
#> Error in explanation_list[order]: invalid subscript type 'closure'
```
...
#> Error in eval(expr, envir, enclos): object 'best_methods' not found
...
#> Error in UseMethod("explain"): no applicable method for 'explain' applied to an object of class "lm"
...
#> Error in explanation_list_mixed[order]: invalid subscript type 'closure'
```
