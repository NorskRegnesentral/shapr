...
First we set up the `airquality` dataset and train the `xgboost` model.
...
plot_MSEv_scores <- function(explanation_list, method_line = NULL) {
  fig <- plot_MSEv_eval_crit(explanation_list) +
    ...
    ggplot2::coord_flip() +
    ggplot2::theme(plot.title = ggplot2::element_text(size = rel(1)))
...
the Monte Carlos based `empirical` approach with defualt hyperparameters.
```{r empirical}
# Compute the Shapley value explanations using the empirical method
explanation_list$empirical <- shapr::explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  approach = "empirical",
...
2. In `regression_tune_values`, we must provide a data.frame/data.table/tibble of the
possible hyperparameter values to consider. The column names of `regression_tune_values`
...
drastically improves the methods. The two cross-validated methods are very comparable, but the second version
...
Thus, we have to evaluate if the better precision is worth the extra elapsed time. 
...
# Using xgboost with default parameters as the surrogate model
explanation_list$sur_xgboost_cv3 <- explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  prediction_zero = p0,
...
plot_MSEv_scores(explanation_list, method_line = "empirical")
...
```{r}
plot_SV_several_approaches(explanation_list, index_explicands = 1:2)
```
...
explanation_list$sep_ppr_cv <- explain(
  ...
  prediction_zero = p0,
  approach = "regression_separate",
  regression_model = parsnip::ppr_reg(num_terms = tune()) %>%
...
explanation_list$sur_ppr <- shapr::explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  prediction_zero = p0,
  approach = "regression_surrogate",
  regression_model = parsnip::ppr_reg(num_terms = 3) %>%
    parsnip::set_engine("ppr") %>%
    parsnip::set_mode("regression")
)

# PPR surrogate with cross-validated number of terms
explanation_list$sur_ppr_cv <- shapr::explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
  prediction_zero = p0,
...
# Compare the Shapley value explanations for the first four explicands
plot_SV_several_approaches(explanation_list_ppr, index_explicands = c(1:4))
```
