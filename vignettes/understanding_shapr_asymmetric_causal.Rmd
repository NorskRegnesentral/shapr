...
are from the Gaussian distribution, and, compared to `CauSHAPley`, our implementation
can take use of all of `shapr`'s new features, such as batching and parallelization.
...
used to compute the observational expectations. The marginals
...
## Code setup
First, we load the needed libraries and set up the training/explicand data, plot the data, and train an `xgboost` model.
```{r setup_1, message = FALSE, fig.height = 4}
# Libraries
library(ggplot2)
require(GGally)
#library(ggpubr)
...
```{r setup_2, message = FALSE, fig.height = 7}
...
```{r setup_3, message = FALSE, fig.height = 4}
# Test/explicand data
...
ggplot(data.table("response" = y_explain[,1], "predicted_response" = predict(model, x_explain)),
       aes(response, predicted_response)) + geom_point()
```
...
print_time <- function(explanation_list) {
  t(sapply(explanation_list, function(explanation) explanation$timing$timing_secs))
}
...
explanation_sym_con[["gaussian"]] <- explain(
  model = model,
  x_train = x_train,
  x_explain = x_explain, n_samples = 5,
  approach = "gaussian",
...
```{r explanation_sym_con_SV, fig.height = 7}
...
```{r explanation_sym_con_beeswarm, fig.height = 9}
...
```{r explanation_asym_con_beeswarm, fig.height = 9}
...
```{r sym_and_asym_Shapley_values, fig.height = 7}
...
```{r explanation_sym_mar_beeswarm, fig.height = 9}
...
See the [implementation details](#Implementation_details) for an explanation for why.
```{r}
print_time(explanation_asym_cau)
```
We can then plot the beeswarm plots. We see that `ctree` provides more spread out Shapley values.


```{r explanation_asym_cau_beeswarm, fig.height = 9}
# Plot the beeswarm plots
plot_beeswarms(explanation_asym_cau, title = "Asymmetric causal Shapley values")
```

```{r explanation_asym_cau_SV, fig.height = 8}
...
```{r compare_plots, cache = TRUE, fig.height = 8}
...
```{r scatter_plots, cache = TRUE, fig.height = 6}
...
# Make the plots
scatterplot_topleft <-
  ggplot(sv_correlation_df, aes(x = sv_framework_1_feature_2, y = sv_framework_1_feature_1, color = color)) +
  geom_point(size = 1) +
...
scatterplot_topright <-
  ggplot(sv_correlation_df, aes(x = sv_framework_2_feature_1, y = sv_framework_1_feature_1, color = color)) +
...
scatterplot_bottomleft <-
  ggplot(sv_correlation_df, aes(x = sv_framework_1_feature_2, y = sv_framework_2_feature_2, color = color)) +
  geom_point(size = 1) + scale_color_gradient(low="blue", high="red") +
  xlab(paste(sv_framework_1_str, feature_2)) +
...
scatterplot_bottomright <-
  ggplot(sv_correlation_df, aes(x = sv_framework_2_feature_1, y = sv_framework_2_feature_2, color = color)) +
  geom_point(size = 1) +
  xlab(paste(sv_framework_2_str, feature_1)) +
  ylab(paste(sv_framework_2_str, feature_2)) +
  scale_x_continuous(limits = c(-1500, 1000), breaks = c(-1000, 0, 1000)) +
...
## Looking at two dates

We investigate the difference between symmetric/asymmetric conditional, symmetric/asymmetric causal, and
marginal Shapley values  for two days: October 10 and December 3, 2012. They have more or less the same
temperature of 13 and 13.27 degrees Celsius, and predicted bike counts of 6117 and 6241, respectively.
The figure below is an extension of Figure 4 in @heskes2020causal, as they only included asymmetric
...
as @heskes2020causal obtained, namely, that the marginal Shapley values similar explanation for both days.
I.e., it only considers the direct effect of `temp`. The asymmetric conditional and causal Shapley values
are almost indistinguishable and put the most weight on the ‘root’ cause `cosyear`. @heskes2020causal
states that the symmetric causal Shapley values provides a sensible balance the two extremes and gives
credit to both season and temperature, but still different explanation for the two days.

However, as we also include symmetric conditional Shapley values, we see that they are extremely similar
to symmetric causal Shapley values. I.e., conditional Shapley values also provides a sensible balance
between marginal and asymmetric Shapley values. To summarize: asymmetric conditional/causal Shapley
values focus on the root cause, marginal Shapley values on the more direct effect, and symmetric 
conditional/causal Shapley consider both for a more natural explanation.

```{r two_dates_1, cache = TRUE, fig.height = 5}
...
explanations = list("Sym. Mar." = explanation_sym_marg[["gaussian"]],
...
# Make the plot
ggplot(dt_all, aes(x = feature, y = value, group = interaction(Date, feature),
...
```{r two_dates_2, cache = TRUE, fig.height = 4}
...
plot_SV_several_approaches(explanations,
                           ...
                           axis_labels_n_dodge = 1) + theme(legend.position = "bottom")
```
...
```{r two_dates_3, cache = TRUE, fig.height = 5}
# Here 2012-10-09 is the left facet and 2012-12-03 the right facet
plot_SV_several_approaches(explanations,
                           index_explicands = dates_idx,
                           facet_scales = "free_x", 
                           ...
                           digits = 2) + theme(legend.position = "bottom")
```


## Sampling of combinations

We can use `n_combination` to specify/reduce the number of feature combinations
...
have fewer valid combinations due to the causal ordering.

In the example below, we demonstrate for asymmetric and symmetric causal 
Shapley values. We half the the number of combinations for both versions
and see that the elapsed times are approximately halfed, too.
```{r n_combinations, cache = TRUE}
explanation_n_comb = list()

explanation_n_comb[["sym_cau_gaussian_64"]] <- explain(
  model = model,
  x_train = x_train,
  x_explain = x_explain, n_samples = 5,
  approach = "gaussian",
  ...
  causal_ordering = list(1, 2:3, 4:7),
  confounding = c(FALSE, TRUE, FALSE),
  n_combinations = 64 # Instead of 128
)

explanation_n_comb[["asym_cau_gaussian_10"]] <- explain(
  model = model,
  x_train = x_train,
  x_explain = x_explain, n_samples = 5,
  approach = "gaussian",
  ...
  causal_ordering = list(1, 2:3, 4:7),
  confounding = c(FALSE, TRUE, FALSE),
  n_combinations = 10 # Instead of 20
)

# Look at the times
explanation_n_comb[["sym_cau_gaussian_all_128"]] = explanation_sym_cau$gaussian
explanation_n_comb[["asym_cau_gaussian_all_20"]] = explanation_asym_cau$gaussian
explanation_n_comb = explanation_n_comb[c(1,3,2,4)]
print_time(explanation_n_comb)
```

We can then plot the beeswarm plots and the Shapley values for the six selected explicands.
We see that their are only minuscule differences between the Shapley values we obtain when we use
all the combinations and those we obtain when we use half of the valid combinations.

```{r n_combinations_plot_beeswarm, cache = TRUE, fig.height = 12}
plot_beeswarms(explanation_n_comb, title = "Shapley values (gaussian) exact vs. approximation")
```

```{r n_combinations_plot_SV, cache = TRUE, fig.height = 8}
plot_SV_several_approaches(explanation_n_comb, index_x_explain) +
  theme(legend.position = "bottom") +
  guides(fill = guide_legend(nrow = 2))
```
...
feature level. We demonstrate using the `gaussian` approach, but other approaches
are applicable, too.
...
```{r group_cor, cache = TRUE, fig.height = 4}
...
We can then compute the Shapley values using the different Shapley value frameworks.
```{r group_gaussian, cache = TRUE}
...
For the beeswarm plots, we set `groupwise_feature_means = TRUE` to make the plots.
...
```{r group_gaussian_plot_beeswarm, cache = TRUE, fig.height = 15}
...
```{r group_gaussian_plot_SV, cache = TRUE, fig.height = 8}
...
S_causal_steps = explanation_sym_cau$gaussian$internal$objects$S_causal_steps
