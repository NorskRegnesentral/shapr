...
```{r scatter_plots, cache = TRUE, fig.height=6, fig.width = 6}
...
```{r two_dates_1, cache = TRUE, fig.height = 7, fig.width = 7}
...
```{r two_dates_2, cache = TRUE, fig.height = 7, fig.width = 7}
...
```{r two_dates_3, cache = TRUE, fig.height = 5, fig.width = 10}
# Here 2012-10-09 is the left facet and 2012-12-03 the right facet
plot_SV_several_approaches(explanations,
                           index_explicands = dates_idx,
                           facet_scales = "free_x", 
...
explanation_group_gaussian[["symmetric_marginal"]] <- progressr::with_progress({
  explain(
    model = model,
    x_train = x_train,
    x_explain = x_explain,
...
explanation_group_gaussian[["symmetric_conditional"]] <- progressr::with_progress({
  explain(
    model = model,
    x_train = x_train,
    x_explain = x_explain,
...
explanation_group_gaussian[["asymmetric_conditional"]] <- progressr::with_progress({
  explain(
    model = model,
    x_train = x_train,
    x_explain = x_explain,
...
explanation_group_gaussian[["symmetric_causal"]] <- progressr::with_progress({
  explain(
    model = model,
    x_train = x_train,
    x_explain = x_explain,
...
explanation_group_gaussian[["asymmetric_causal"]] <- progressr::with_progress({
  explain(
    model = model,
    x_train = x_train,
    x_explain = x_explain,
...
```{r group_gaussian_plot_SV, cache = TRUE, fig.height = 8, fig.width = 9}
plot_beeswarms(explanation_group_gaussian, title = "Group Shapley values (gaussian)", groupwise_feature_means = TRUE)
