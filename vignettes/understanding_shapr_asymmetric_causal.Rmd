...
# Look at the elapsed times
print_time(explanation_group_gaussian)
```

We then make the beeswarm plots and Shapley values plots for the six selected explicands.
For the beeswarm plots, we set `groupwise_feature_means = TRUE` to make the plots. This means that
we the plot function use the mean of the features in the groups.
...
plot_SV_several_approaches(explanation_group_gaussian, index_x_explain, groupwise_feature_means = TRUE)
```
