...
knitr::opts_chunk$set(
  ...
  fig.width = 9,
  fig.height = 5,
  fig.path = "figure_general_usage/", # Ensure that figures are saved in the right folder (this vignette will be built manually)
  cache.path = "cache_general_usage/", # Ensure that cached objects are saved in the right folder
  warning = FALSE,
  message = TRUE,
  tidy = "styler"
)
...
| `summary.shapr`    | Prints and returns a formatted summary of the Shapley value computation.                  |
...
```{r setup-2, warning=FALSE, cache = TRUE}
library(xgboost)
library(data.table)
...
```{r plot-waterfall, warning=FALSE, cache = TRUE}
...
```{r plot-beeswarm, warning=FALSE, cache = TRUE}
...
```{r plot-scatter, warning=FALSE, cache = TRUE}
...
```{r factor, warning=FALSE, cache = TRUE}
...
```{r ctree, warning=FALSE, cache = TRUE}
...
```{r categorical, warning=FALSE, cache = TRUE}
...
```{r setup-MSE, warning=FALSE, cache = TRUE}
...
```{r MSEv-plot, warning=FALSE, cache = TRUE}
...
```{r MSEv-plot-2, warning=FALSE, cache = TRUE}
...
```{r MSEv-plot-3, warning=FALSE, cache = TRUE}
...
```{r MSEv-plot-4, warning=FALSE, cache = TRUE}
...
```{r iterative, warning=FALSE, cache = TRUE}
library(xgboost)
library(data.table)
...
As shown above, `print(explain_object)` simply prints the estimated Shapley values from an object computed with 
`explain()`. 
This is the default behavior of the `print()` method for `shapr` objects.
However, `print.shapr` has a `what` argument which controls what is printed, allowing
the user to the standard deviations stemming from the coalition sampling (`what = shapley_sd`), and variants of the
[MSEv criterion](#MSEv) introduced above via `what = "MSEv"/"MSEv_explicand"/"MSEv_coalition"`.

Moreover, `summary(explain_object)` gives a nicely formatted summary of the shapr object, and invisibly returns a 
full list of summary components. 
In the background `summary.shapr` function calls `get_results()` which provides this full list of summary components,
including which parameters are being used, the intermediately estimated Shapley values (and sd), estimates of all the 
`v(S)`, and so on. 
See `?get_results` for the full list of returned objects. 

Below we illustrate some of these capabilities using the iterative explanation computation above.


```{r summary, warning=FALSE, cache = TRUE}
# Uses the object ex computed to illustrate printing and summary functionality
print(ex) # Prints the estimated Shapley values by default
...
summary_ex$shapley_sd # The estimated standard deviations of the Shapley values
summary_ex$timing$total_time_secs # Total computation time in seconds
summary_ex$parameters$n_MC_samples # Number of Monte Carlo samples used for the numerical integration

summary_ex$parameters$gaussian.mu # The estimated mean vector used for the Gaussian approach
summary_ex$parameters$gaussian.cov_mat # The estimated covariance matrix used for the Gaussian approach

res_ex <- get_results(ex) # Same as summary(ex)

# May also extract individual summary components directly from get_results() 
get_results(ex, what = "shapley_sd")
get_results(ex, what = "dt_vS")
...
```{r para, warning=FALSE, cache = TRUE}
library(future)
...
```{r combined-1, warning=FALSE, cache = TRUE}
library(xgboost)
library(data.table)
...
```{r combined-2, warning=FALSE, cache = TRUE}
# Use the combined approach
explanation_combined <- explain(
  model = model,
  x_explain = x_explain,
...
```{r group, warning=FALSE, cache = TRUE}
...
```{r custom,warning=TRUE, message=TRUE, cache = TRUE}
library(gbm)
