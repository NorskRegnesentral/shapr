...
Monte Carlo-based approaches (`arf`, `categorical`, `copula`, `ctree`, `empirical`, `gaussian`, `vaeac`,
and `independence` (not recommended)), while the extension of the asymmetric
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 21:58:11 ──────────────────────────
...
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d26551452.rds'
#>
#>
#>
...
#>
#> ℹ Coalitions split into 12 batches (mean 2.2 per batch).
#>
#>
#>
#> ── Iteration 3 ─────────────────────────────────────────────────────────────────
...
#>
#> ℹ Coalitions split into 20 batches (mean 2.3 per batch).
#>
#>
#>
#> ── Iteration 4 ─────────────────────────────────────────────────────────────────
...
#>
#> ℹ Coalitions split into 24 batches (mean 2.9 per batch).
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 21:58:22 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#> • Number of observations to explain: 144
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d6bdca0e1.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#> ── Iteration 3 ─────────────────────────────────────────────────────────────────
#> ℹ Using 46 of 128 coalitions, 20 new.
#> ℹ Coalitions split into 10 batches (mean 4.6 per batch).
#>
#> ── Iteration 4 ─────────────────────────────────────────────────────────────────
#> ℹ Using 70 of 128 coalitions, 24 new.
#> ℹ Coalitions split into 10 batches (mean 7 per batch).
#>
#> ── Iteration 5 ─────────────────────────────────────────────────────────────────
#> ℹ Using 94 of 128 coalitions, 24 new.
#> ℹ Coalitions split into 10 batches (mean 9.4 per batch).
#>
#> ── Iteration 6 ─────────────────────────────────────────────────────────────────
#> ℹ Using 108 of 128 coalitions, 14 new.
#> ℹ Coalitions split into 10 batches (mean 10.8 per batch).
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 21:59:48 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#> • Number of observations to explain: 144
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d17e483b3.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#> ℹ Coalitions split into 10 batches (mean 11 per batch).
#>
#> ── Iteration 7 ─────────────────────────────────────────────────────────────────
#> ℹ Using 114 of 128 coalitions, 4 new.
#> ℹ Coalitions split into 4 batches (mean 28.5 per batch).
```
...
#> gaussian 1093465   71390       11.73
#> ctree    1060934   63108       85.45
#> xgboost  1090931   58988       17.22
```
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:07 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
#> ℹ Capped `max_batch_size` from 10 to 1 so each batch array stays under
#>   `max_batch_cube_size` = 1e+06 elements.
#>
#>
...
#>
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d4d9bd7f.rds'
#>
#>
#>
#> ── Iterative computation started ──
...
#>
#> ℹ Coalitions split into 7 batches (mean 2.9 per batch).
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:11 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
...
#> • Causal ordering: {trend}, {cosyear, sinyear}, {temp, atemp, windspeed, hum}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d7561fb20.rds'
#>
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:13 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
...
#> • Causal ordering: {trend}, {cosyear, sinyear}, {temp, atemp, windspeed, hum}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d239e1d0c.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:30 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
...
#> • Causal ordering: {trend}, {cosyear, sinyear}, {temp, atemp, windspeed, hum}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d67aaa5f7.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#>                          MSEv MSEv_sd Time (secs)
#> gaussian               338742   38645        3.76
#> gaussian_non_iterative 340201   38237        1.73
#> ctree                  282686   32264       17.86
#> xgboost                321763   39778        3.39
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:36 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#> • Causal ordering: {trend, cosyear, sinyear, temp, atemp, windspeed, hum}
#>
#> • Components with confounding: {trend, cosyear, sinyear, temp, atemp,
#> windspeed, hum}
#>
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d6ee043a4.rds'
#>
#>
#>
#> ── Iterative computation started ──
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:46 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#> windspeed, hum}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d3bbe7040.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:00:56 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#> • Number of observations to explain: 144
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d2227e120.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#>                      MSEv MSEv_sd Time (secs)
#> gaussian          1396383  109134        9.97
#> independence_marg 1390794  107977        9.99
#> independence_con  1390287  107899       14.41
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:01:12 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#>
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d6ba88c75.rds'
#>
#>
#>
#> ── Main computation started ──
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:01:45 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 128`, and is
#>   therefore set to `2^n_features = 128`.
...
#> • Components with confounding: {cosyear, sinyear}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d99f6b4c.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#>             MSEv MSEv_sd Time (secs)
#> gaussian 1147000   83388       32.67
#> copula   1164823   86140       30.97
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:02:17 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
...
#>
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d17137421.rds'
#>
#>
#>
#> ── Iterative computation started ──
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:02:22 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
...
#> • Components with confounding: {cosyear, sinyear}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d4ac2178.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ─────────────────────────────────────────────────────────────────
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:02:27 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (20), and is therefore set to 20.
...
#> • Components with confounding: {cosyear, sinyear}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d2db7e197.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 20 of 20 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:09:28 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ Capped `max_batch_size` from 10 to 1 so each batch array stays under
#>   `max_batch_cube_size` = 1e+06 elements.
...
#>
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d517e5cc4.rds'
#>
#>
#>
#> ── Iterative computation started ──
...
#>
#> ℹ Coalitions split into 6 batches (mean 8.7 per batch).
#>
#>
#>
#> ── Iteration 5 ─────────────────────────────────────────────────────────────────
...
#>
#> ℹ Coalitions split into 6 batches (mean 9.7 per batch).
#>
#>
#>
#> ── Iteration 6 ─────────────────────────────────────────────────────────────────
...
#>
#> ℹ Coalitions split into 6 batches (mean 10.7 per batch).
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:09:50 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ Capped `max_batch_size` from 10 to 1 so each batch array stays under
#>   `max_batch_cube_size` = 1e+06 elements.
...
#> • Components with confounding: {cosyear, sinyear}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d351ac3a2.rds'
#>
#> ── Main computation started ──
#>
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:09:56 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_groups = 64`, and is
#>   therefore set to `2^n_groups = 64`.
#> ℹ Capped `max_batch_size` from 10 to 1 so each batch array stays under
#>   `max_batch_cube_size` = 1e+06 elements.
#>
#>
...
#>
#> • Components with confounding: {trend, cosyear, sinyear, temp_group, windspeed,
#> hum}
#>
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d75d9d315.rds'
#>
#>
#>
#> ── Main computation started ──
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:10:04 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_groups = 64`, and is
#>   therefore set to `2^n_groups = 64`.
...
#> • Number of observations to explain: 144
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d439f27b1.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 64 of 64 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:10:09 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (12), and is therefore set to 12.
#> ℹ Capped `max_batch_size` from 10 to 1 so each batch array stays under
#>   `max_batch_cube_size` = 1e+06 elements.
#> ── Explanation overview ──
#>
...
#> • Causal ordering: {trend}, {cosyear, sinyear}, {temp_group, windspeed, hum}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d46232f3b.rds'
#>
#> ── Main computation started ──
#>
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:10:10 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_groups = 64`, and is
#>   therefore set to `2^n_groups = 64`.
...
#> • Components with confounding: {cosyear, sinyear}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d1625a9cd.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 64 of 64 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-07 22:10:23 ──────────────────────────
#> ℹ Feature classes extracted from the model contain `NA`.
#>   Assuming feature classes from the data are correct.
#> ℹ `max_n_coalitions` is `NULL` or larger than the number of coalitions
#>   respecting the causal ordering (12), and is therefore set to 12.
...
#> • Components with confounding: {cosyear, sinyear}
#> • Computations (temporary) saved at:
#> '/tmp/RtmpuzSVRD/shapr_obj_8cd3d50b4387f.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 12 of 12 coalitions.
