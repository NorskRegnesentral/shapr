...
nine different Monte Carlo-based approaches for estimating the conditional distributions of the data, namely
`"empirical"`, `"gaussian"`, `"copula"`, `"ctree"`, `"vaeac"`, `"categorical"`, `"arf"`,
`"timeseries"`, and `"independence"`.
...
Moreover, the default behavior is to estimate the Shapley values iteratively, with an increasing number of
feature coalitions being added, and to stop estimation as the estimated Shapley values have achieved a certain level of
stability.
...
The package provides functionality for printing and summarizing the Shapley value computation, as well as extracting
objects created during computation, see [Summary, Printing, and Result Extraction](#summary).
...
By default `explain` always computes feature-wise Shapley values.
Groups of features can be explained by providing the feature groups through the `group` argument.

When there are five or fewer features (or feature groups), iterative estimation is by default disabled.
...
As opposed to Kernel SHAP estimation as originally proposed in (@lundberg2017unified), `shapr`, by reweights the samples according to
the "Paired c-kernel" strategy described in @olsen2025improving to reduce the estimation variance and thereby improve the convergence speed.
...
Details on this are found in @aas2019explaining.
...
mixed (i.e., numerical, categorical, discrete, and ordinal) features and both
...
## Adversarial Random Forest (arf) Approach {#arf}

Another approach that supports mixed features, including factor features,
is the adversarial random forest approach, abbreviated to `arf`. The `arf`
approach first fits an adversarial random forest to the training data and
then uses the fitted forest to estimate the joint feature distribution. We
use this estimated distribution to generate Monte Carlo samples from
$p(\boldsymbol{x}_{\bar{\mathcal{S}}} \mid \boldsymbol{x}_{\mathcal{S}}^*)$
for each coalition. The resulting samples are then used to approximate the
conditional expectations in the same Kernel SHAP framework as the other
Monte Carlo-based approaches. The `arf` approach is implemented through the
*arf* package and can be a flexible alternative for data sets containing both
...
`"categorical"`, `"arf"`, `"timeseries"`, `"independence"`, `"regression_separate"`, and
...
The `waterfall` option shows the changes in the prediction score due to each feature's contribution (their Shapley values):
...
We can use mixed (i.e., continuous, categorical, discrete, and ordinal) data with `ctree`, `arf`, or `vaeac`.
...
The `arf` approach can be used with the same mixed feature data, including
the factor feature `Month_factor`. For a small example, we reduce the number
of Monte Carlo samples and the number of trees.
...
## Choosing a conditional approach {#choosing-a-conditional-approach}

`shapr` provides several approaches for estimating the contribution functions
$v(S)$, with different trade-offs in estimation accuracy and computational
cost.
For practical guidance on runtime and memory use, see the
[computational cost benchmarks](https://norskregnesentral.github.io/shapr/articles/benchmarks.html),
which compare the available approaches and other arguments.
When computationally feasible, we recommend comparing candidates on the
data at hand using the [$\operatorname{MSE}_{v}$ evaluation criterion](#MSEv).
Section 6 of
[Olsen et al. (2024)](https://doi.org/10.1007/s10618-024-01016-z)
provides general guidance that can help identify plausible candidates,
and is the recommended source; below we only provide a brief summary.
Olsen et al. (2024) found that `independence` is preferable only when
all feature dependence is absent or negligible.
Otherwise, a correctly or nearly
correctly specified parametric approach, such as `gaussian` or `copula`, generally performed best.
When reasonable parametric assumptions were unavailable, the studied generative approaches, `ctree` and `vaeac`, or
a suitably flexible `regression_separate` approach were often good alternatives.
Note that the newer `arf` approach was not evaluated by Olsen et al. (2024).

## MSEv evaluation criterion {#MSEv}
...
coalitions is used).
The default is to use uniform weighting, but this can be changed by passing `list(MSEv_uniform_comb_weights = FALSE)`
to the `output_args` argument of the `explain()` function.
...
we have only averaged over the observations or the coalitions (both as bar and line plots).
...
We can specify the `index_x_explain` and `id_coalition` parameters in `plot_MSEv_eval_crit()` to only plot
certain test observations and coalitions, respectively.
...
If the variances are too high, we estimate the number of required samples to reach convergence, and thereby add more coalitions.
...
# Summary, Printing, and Result Extraction {#summary}

As shown above, `print(explain_object)` displays the estimated Shapley values from an object created using `explain()`.
This is the default behavior of the `print()` method for `shapr` objects.

However, `print.shapr` also includes a `what` argument that controls what is printed.
This allows users to also display:
- the standard deviations arising from coalition sampling (`what = "shapley_sd"`), and
- variants of the [MSEv criterion](#MSEv) introduced above, using
  `what = "MSEv"`, `"MSEv_explicand"`, or `"MSEv_coalition"`.

The `summary(explain_object)` function returns a `summary.shapr` object containing a complete list of summary components.
When called interactively without assignment (or explicitly printed with `print()`), it displays a nicely formatted
summary to the console. When assigned to a variable, the formatted output is suppressed, but the returned object can
be used to extract individual components later.
Internally, `summary.shapr` calls `get_results()`, which produces this full set of components, including:
the parameters used, intermediate Shapley value estimates (and their standard deviations), estimates of all \( v(S) \),
and more.
See `?get_results` for a full overview of the returned output.

Below, we illustrate these capabilities using the iterative explanation object `ex` computed above.
...
# SAGE: global feature importance {#sage}

The Shapley value explanations described above are *local*: they explain a single prediction at a time.
Sometimes we are instead interested in a *global* measure of how much each feature contributes to the overall
predictive performance of the model. SAGE (Shapley Additive Global importancE,
[Covert et al. (2020)](https://proceedings.neurips.cc/paper/2020/file/c7bf0b7c1a86d5eb3be2c722cf2cf746-Paper.pdf))
provides exactly this, by applying the Shapley value framework to a model *loss* rather than to the prediction itself.

Concretely, the value of a coalition \( S \) becomes \( v(S) = -\mathbb{E}[L(y, f(x_S))] \), the (negative) expected
loss of the model when only the features in \( S \) are known. The conditional expectations \( f(x_S) = E[f(x)\mid x_S] \)
are estimated with the same `approach` machinery as for ordinary Shapley values, so all approaches, groups and
iterative estimation are available for SAGE too. The resulting SAGE values are a single set of feature importances
that sum to the difference between the loss of the full model and the loss of the baseline prediction `phi0`.

To compute SAGE values, set `scope = "global"` and provide the response vector `y_explain` corresponding to the rows of
`x_explain`. The loss function is controlled through `extra_computation_args$global_loss_func`; if not supplied, `shapr` uses logistic
(cross-entropy) loss for binary 0/1 responses and mean squared error otherwise.
...
The ordinary per-observation Shapley value explanations of the predictions are still computed and stored alongside
the SAGE values in `sage_explanation$internal$output$shap_values_est`, in case they are of interest.

Since SAGE reuses the regular Shapley value machinery and only swaps out the value function, `scope = "global"` also
works together with the grouping, causal and asymmetric functionality described elsewhere in this vignette and in the
[asymmetric and causal Shapley values](asymmetric_causal.html) vignette. Passing `group` gives one global SAGE value
per group, while passing `causal_ordering` (optionally with `confounding`) and/or `asymmetric = TRUE` attributes the
global loss reduction according to the specified (partial) causal ordering. These causal and asymmetric SAGE
combinations are a natural extension of the SAGE methodology rather than variants described in the original SAGE paper.
...
work on all OSes, allow the user to decide the parallelization
backend (multiple R processes or forking), work directly with HPC
...
The `vaeac` approach is an exception to the general backend support. Its `torch` objects contain external pointers
that cannot be exported to separate R processes, so `multisession` and `cluster` plans with more than one worker are
unsupported. Use `future::plan(future::multicore)` where forking is available, or run `vaeac` sequentially. Forking
is unavailable on Windows and within RStudio.
...
elements. Note, only one class per feature is supported and for ordered
factors the element should be "factor" and not "ordered"), and
...
data passed to `explain` have the correct format in terms
...
`xgboost` (i.e., `parsnip::boost_tree`) with any other fitted `tidymodels` model in the `workflows` procedure outlined below.
...
There are additional extra parameters which can be set by including a named list in the call to
the `explain()` function. For example, we can change the batch size to 32 by including
`vaeac.extra_parameters = list(vaeac.batch_size = 32)` as a parameter in the call to the `explain()` function. See `?shapr::vaeac_get_extra_para_default` for a description of the possible
extra parameters to the `vaeac` approach.
...
We can look at the training and validation errors for the trained `vaeac` model and see that `vaeac.epochs = 3`
is likely too few epochs, as it still seems like the `vaeac` model is learning.
...
and in the `vaeac.extra_parameters` list we set `vaeac.epochs_early_stopping` to the number of epochs we
...
for `2` consecutive epochs, or if `vaeac.epochs` is reached. Note that if using early stopping and progress
...
be more informative to explain the impact of each variable, rather
