...
test = sample_unit_development(x[,1,], X[["features"]], nsamples = 1e4, sigma)

tester <- data.table::data.table(ID = rep(1:1e4, 1e4))
tester[, n := runif(.N)]
data.table::setkey(tester, ID, n)
tester[, n := n/sum(n), ID]
tester[, s := cumsum(n), ID]
