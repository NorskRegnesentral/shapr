...
Z <- cbind(1,l$S)
W <- diag(l$X$shapley_weight)
v <- DT[id==1,k]

save(Z,W,v,file = "constrained_problem_test.RData")


###############
phi <- solve(t(Z) %*%W %*% Z) %*% t(Z) %*%W %*%v # Manual version of solving WLS problem

sum(phi)-v[64] # identical
phi[1]-v[1] # identical

# Making constrained matrix
E <- matrix(0,ncol=length(phi),nrow=2)
E[1,1]=1
E[2,]=1
...
test=lsei(A=Z[-c(1,64),],
           B=v[-c(1,64)],
           E=E,
           F=F,
           Wa=sqrt(diag(W)[-c(1,64)]))

as.vector(E%*%test$X)- F # Identical
...
nrows_prop_vec <- c(0.5,1,2,5,10,50,100,500,1000)
a <- 1
...
for (nrows_prop in nrows_prop_vec){
    ...
    for (i in l$Xtest[, .I]) {
        ...
        for (j in 1:length(Shapley.approx)){
            #####
            feature_list <- Shapley.approx[[j]]$l$X$features
            ...
            Shapley.approx[[j]]$Kshap[i,] <- Shapley.approx[[j]]$l$W %*% lll[id == i, k]
        }
    }
    ...
    plot(all.res.DT[rn=="with_replacement_no_shapley_weights",nrows_used],all.res.DT[rn=="with_replacement_no_shapley_weights",absmean_total],type="l",xlim=c(0,2^X_dim),ylim=range(all.res.DT$absmean_total))
