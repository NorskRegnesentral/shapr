...
dependence-aware Shapley values with nine different Monte Carlo-based
approaches for estimating the conditional distributions of the data,
namely `"empirical"`, `"gaussian"`, `"copula"`, `"ctree"`, `"vaeac"`,
`"categorical"`, `"arf"`, `"timeseries"`, and `"independence"`. `shapr`
has also implemented two regression-based approaches
`"regression_separate"` and `"regression_surrogate"`. See [Estimation
approaches and plotting functionality](#ex) below for examples. It is
also possible to combine the different approaches, see the [combined
approach](#combined).
...
mixed (i.e., numerical, categorical, discrete, and ordinal) features and
...
### Adversarial Random Forest (arf) Approach

Another approach that supports mixed features, including factor
features, is the adversarial random forest approach, abbreviated to
`arf`. The `arf` approach first fits an adversarial random forest to the
training data and then uses the fitted forest to estimate the joint
feature distribution. We use this estimated distribution to generate
Monte Carlo samples from
$`p(\boldsymbol{x}_{\bar{\mathcal{S}}} \mid \boldsymbol{x}_{\mathcal{S}}^*)`$
for each coalition. The resulting samples are then used to approximate
the conditional expectations in the same Kernel SHAP framework as the
other Monte Carlo-based approaches. The `arf` approach is implemented
through the *arf* package and can be a flexible alternative for data
sets containing both numeric and factor features.
...
`"timeseries"`, `"independence"`, `"regression_separate"`, and
...
# Computing the actual Shapley values with Kernel SHAP accounting for feature
# dependence using the empirical (conditional) distribution approach with bandwidth
# parameter sigma = 0.1 (default)
explanation <- explain(
  model = model,
  x_explain = x_explain,
  x_train = x_train,
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:08:54 ───────────────────────────────
...
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678399fb426.rds'
#>
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:08:58 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736785ce6a3be.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
We can use mixed (i.e., continuous, categorical, discrete, and ordinal)
data with `ctree`, `arf`, or `vaeac`. Use `ctree` with mixed data in the
following manner:
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:10 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678777ad3f3.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:12 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736785e6f4567.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
The `arf` approach can be used with the same mixed feature data,
including the factor feature `Month_factor`. For a small example, we
reduce the number of Monte Carlo samples and the number of trees.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:13 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736786e0cd3b7.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
![](figure_general_usage/arf-1.webp)
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:16 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678a83061c.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
# Creating a predictive model (for illustration just predicting the next point in
# the time series with a linear model)
...
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367841dfd0cf.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
functions $`v(S)`$, with different trade-offs in estimation accuracy and
computational cost. For practical guidance on runtime and memory use,
see the [computational cost
benchmarks](https://norskregnesentral.github.io/shapr/articles/benchmarks.html),
which compare the available approaches and other arguments. When
computationally feasible, we recommend comparing candidates on the data
at hand using the [$`\operatorname{MSE}_{v}`$ evaluation
criterion](#MSEv). Section 6 of [Olsen et
al. (2024)](https://doi.org/10.1007/s10618-024-01016-z) provides general
guidance that can help identify plausible candidates, and is the
recommended source; below we only provide a brief summary. Olsen et
al. (2024) found that `independence` is preferable only when all feature
dependence is absent or negligible. Otherwise, a correctly or nearly
correctly specified parametric approach, such as `gaussian` or `copula`,
generally performed best. When reasonable parametric assumptions were
unavailable, the studied generative approaches, `ctree` and `vaeac`, or
a suitably flexible `regression_separate` approach were often good
alternatives. Note that the newer `arf` approach was not evaluated by
Olsen et al. (2024).
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:19 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736787be82cff.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:20 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 25
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678a6dcc3.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:25 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 25
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367825ffe76.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 25
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736787ab53dd4.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 25
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678350dfcf1.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
# The main plot of the overall MSEv averaged over both the coalitions and
# observations
MSEv_plots$MSEv_bar
```
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:30 ───────────────────────────────
...
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367837420036.rds'
#>
...
#> • Computed with `shapr::explain()` in 4.2 seconds, started 2026-09-11 13:09:30
#> • Model class: <xgboost>
#> • v(S) estimation class: Monte Carlo integration
#> • Approach: gaussian
#> • Procedure: Iterative
...
# Alternatively, use get_results() to get the same components without printing the
# summary
...
The Shapley value explanations described above are *local*: they explain
a single prediction at a time. Sometimes we are instead interested in a
*global* measure of how much each feature contributes to the overall
predictive performance of the model. SAGE (Shapley Additive Global
importancE, [Covert et
al. (2020)](https://proceedings.neurips.cc/paper/2020/file/c7bf0b7c1a86d5eb3be2c722cf2cf746-Paper.pdf))
provides exactly this, by applying the Shapley value framework to a
model *loss* rather than to the prediction itself.

Concretely, the value of a coalition $`S`$ becomes
$`v(S) = -\mathbb{E}[L(y, f(x_S))]`$, the (negative) expected loss of
the model when only the features in $`S`$ are known. The conditional
expectations $`f(x_S) = E[f(x)\mid x_S]`$ are estimated with the same
`approach` machinery as for ordinary Shapley values, so all approaches,
groups and iterative estimation are available for SAGE too. The
resulting SAGE values are a single set of feature importances that sum
to the difference between the loss of the full model and the loss of the
baseline prediction `phi0`.

To compute SAGE values, set `scope = "global"` and provide the response
vector `y_explain` corresponding to the rows of `x_explain`. The loss
function is controlled through
`extra_computation_args$global_loss_func`; if not supplied, `shapr` uses
logistic (cross-entropy) loss for binary 0/1 responses and mean squared
error otherwise.

``` r

# Compute SAGE values explaining the model's global mean squared error over the training data
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:35 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 32`, and is therefore
#>   set to `2^n_features = 32`.
...
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736786369acfe.rds'
#>
#> ── Main computation started ──
#>
...
![](figure_general_usage/sage-1.webp)

``` r

plot(sage_explanation, plot_type = "waterfall")
```

![](figure_general_usage/sage-2.webp)

The ordinary per-observation Shapley value explanations of the
predictions are still computed and stored alongside the SAGE values in
`sage_explanation$internal$output$shap_values_est`, in case they are of
interest.

Since SAGE reuses the regular Shapley value machinery and only swaps out
the value function, `scope = "global"` also works together with the
grouping, causal and asymmetric functionality described elsewhere in
this vignette and in the [asymmetric and causal Shapley
values](https://norskregnesentral.github.io/shapr/articles/asymmetric_causal.md)
vignette. Passing `group` gives one global SAGE value per group, while
passing `causal_ordering` (optionally with `confounding`) and/or
`asymmetric = TRUE` attributes the global loss reduction according to
the specified (partial) causal ordering. These causal and asymmetric
SAGE combinations are a natural extension of the SAGE methodology rather
than variants described in the original SAGE paper.
...
you may want to limit the number of workers for that reason too.

The `vaeac` approach is an exception to the general backend support. Its
`torch` objects contain external pointers that cannot be exported to
separate R processes, so `multisession` and `cluster` plans with more
than one worker are unsupported. Use `future::plan(future::multicore)`
where forking is available, or run `vaeac` sequentially. Forking is
unavailable on Windows and within RStudio.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:40 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 32`, and is therefore
#>   set to `2^n_features = 32`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 5
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367827954fa1.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 32 of 32 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:45 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367829b77628.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:47 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736782a5e52eb.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:49 ───────────────────────────────
...
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736785f6c4fdd.rds'
#>
#> ── Main computation started ──
#>
...
(a named vector with the labels as names and the class type as elements.
Note, only one class per feature is supported and for ordered factors
the element should be “factor” and not “ordered”), and *factor_levels*
(a named list with the labels as names and vectors with the factor
levels as elements (NULL if the feature is not a factor)). The
`get_model_specs` function is used to check that the data passed to
`explain` have the correct format in terms of the necessary feature
columns being available and having the correct class/attributes. It is
highly recommended to do such checks in order to ensure correct usage of
`explain`. If, for some reason, such checking is not desirable, one does
not have to provide the `get_model_specs` function. This will, however,
throw a warning that all feature consistency checking against the model
is disabled.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:50 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736787a5c148b.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:54 ───────────────────────────────
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367814b73428.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:09:59 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367857d37d8b.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:10:04 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367871332bac.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:10:19 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 16`, and is therefore
#>   set to `2^n_features = 16`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 4
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678552fe5a.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 16 of 16 coalitions.
...
# First we run the computation with the iterative estimation procedure for a limited
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:10:43 ───────────────────────────────
#>
#> ── Explanation overview ──
#>
#> • Model class: <xgboost>
...
#> • Number of feature-wise Shapley values: 5
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736789286863.rds'
#>
#> ── Iterative computation started ──
#>
#> ── Iteration 1 ──────────────────────────────────────────────────────────────────────
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:10:49 ───────────────────────────────
#>
#> ── Explanation overview ──
#>
#> • Model class: <xgboost>
...
#> • Number of feature-wise Shapley values: 5
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736786c21428e.rds'
#>
#> ── Main computation started ──
#>
...
# Using the ex_init object to continue the computation for the remaining coalition
# samples but this time using the path to the saved intermediate estimation object
...
#>
#> ── Starting `shapr::explain()` at 2026-09-11 13:10:50 ───────────────────────────────
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 32`, and is therefore
#>   set to `2^n_features = 32`.
#>
#> ── Explanation overview ──
...
#> • Number of feature-wise Shapley values: 5
#> • Number of observations to explain: 6
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_273678336fb7ef.rds'
#>
#> ── Main computation started ──
#>
...
#>
#> ── Starting `shapr::explain_forecast()` at 2026-09-11 13:10:52 ──────────────────────
...
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367818cd055f.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 4 of 4 coalitions.
...
#>
#> ── Starting `shapr::explain_forecast()` at 2026-09-11 13:10:55 ──────────────────────
#> ℹ Feature names extracted from the model contain `NA`.
#>   Consistency checks between model and data are therefore disabled.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 4`, and is therefore
#>   set to `2^n_features = 4`.
...
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_2736785e4e70eb.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 4 of 4 coalitions.
...
#>
#> ── Starting `shapr::explain_forecast()` at 2026-09-11 13:10:56 ──────────────────────
#> ℹ Feature names extracted from the model contain `NA`.
#>   Consistency checks between model and data are therefore disabled.
#> ℹ `max_n_coalitions` is `NULL` or larger than `2^n_features = 32`, and is therefore
#>   set to `2^n_features = 32`.
...
#> • Number of feature-wise Shapley values: 5
#> • Number of observations to explain: 1
#> • Computations (temporary) saved at: '/tmp/RtmpcavhZb/shapr_obj_27367863a29192.rds'
#>
#> ── Main computation started ──
#>
#> ℹ Using 32 of 32 coalitions.
...
#>
#> ── Summary of Shapley value explanation ─────────────────────────────────────────────
#> • Computed with `shapr::explain_forecast()` in 4.1 seconds, started 2026-09-11
#> 13:10:56
#> • Model class: <ar>
#> • v(S) estimation class: Monte Carlo integration
#> • Approach: empirical
