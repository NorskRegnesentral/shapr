...
data[, Month_factor_ordered := as.ordered(Month_factor)]
...
x_var_mixed_ordered <- c("Solar.R", "Wind", "Temp", "Day", "Month_factor_ordered")
...
x_train_mixed_ordered <- data_train[, ..x_var_mixed_ordered]
...
x_explain_mixed_ordered <- data_explain[, ..x_var_mixed_ordered]
...
lm_formula_mixed_ordered <- as.formula(paste0(y_var_numeric, " ~ ", paste0(x_var_mixed_ordered, collapse = " + ")))
...
model_lm_mixed_ordered <- lm(lm_formula_mixed_ordered, data = data_complete)
