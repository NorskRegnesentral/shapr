...
(absmeans.comb = colMeans(abs(Shapley.true$exactShap[,-1]-Shapley.approx$comb$Kshap[,-1])))



# Mean of the absolute errors over all variables
res_to_paper <- data.frame(S_KS=mean(absmeans.indep),G_KS = mean(absmeans.Gauss),C_KS = mean(absmeans.copula),COMB_KS = mean(absmeans.comb),E_KS_0.1=mean(absmeans.sigma.01),E_KS_0.3=mean(absmeans.sigma.03),E_KS_AICc=mean(absmeans.sigma.AICc),TreeSHAP = NA)
