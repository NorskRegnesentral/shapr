...
(absmeans.comb = colMeans(abs(Shapley.true$exactShap[1:10,-1]-Shapley.approx$comb$Kshap[1:10,-1])))
...
# Mean absolute errors per variable (to see if the performance differ between variables)
(absmeans.sigma.01 = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.approx$sigma.01$Kshap[1:50,-1])))
(absmeans.sigma.03 = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.approx$sigma.03$Kshap[1:50,-1])))
(absmeans.indep = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.approx$indep$Kshap[1:50,-1])))
(absmeans.Gauss = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.approx$Gauss$Kshap[1:50,-1])))
(absmeans.copula = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.approx$copula$Kshap[1:50,-1])))
(absmeans.comb = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.approx$comb$Kshap[1:50,-1])))
(absmeans.treeSHAP = colMeans(abs(Shapley.true$exactShap[1:50,-1]-Shapley.tree[1:50,-1]))) # Additional one



# Mean of the absolute errors over all variables
# Mean of the absolute errors over all variables
res_to_paper <- data.frame(S_KS=mean(absmeans.indep),G_KS = mean(absmeans.Gauss),C_KS = mean(absmeans.copula),E_KS_0.1=mean(absmeans.sigma.01),E_KS_0.3=mean(absmeans.sigma.03),TreeSHAP = mean(absmeans.treeSHAP),comb_KS =mean(absmeans.comb))
spec <- data.frame(gx="Piecewise constant+linear",fx="XGBoost",px="genHyp_heavytails",rho=NA)
(res_to_paper <- cbind(spec,res_to_paper))
...
setkey(Shapley.approx$comb$other_objects$DT,"wcomb")
#setkey(Shapley.approx$sigma.AICc$other_objects$DT,"wcomb")
#setkey(Shapley.approx$sigma.AICc.ind$other_objects$DT,"wcomb")
#setkey(Shapley.approx$sigma.optimal.sigmaval$other_objects$DT,"wcomb")
...
DT.new[,k.comb :=Shapley.approx$comb$other_objects$DT$k]
...
DT.new[id %in% 1:50,.(abserror.Gauss=round(mean(abs(k.true-k.Gauss)),6),
                      abserror.sigma.0.1=round(mean(abs(k.true-k.sigma.0.1)),6),
                      abserror.comb=round(mean(abs(k.true-k.comb)),6),
...
DT.new[id %in% 1:10 & nfeatures == 1,.(abserror.Gauss=round((abs(k.true-k.Gauss)),6),
                                       abserror.sigma.0.1=round((abs(k.true-k.sigma.0.1)),6),
                                       abserror.comb=round((abs(k.true-k.comb)),6),
                                       abserror.sigma.AICc=round((abs(k.true-k.sigma.AICc)),6),
                                       abserror.sigma.AICc.ind=round((abs(k.true-k.sigma.AICc.ind)),6),
                                       abserror.sigma.optimal.2=round((abs(k.true-k.sigma.optimal.2)),6))]
