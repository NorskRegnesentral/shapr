...
* Added the `extra_computation_args$max_batch_cube_size` argument, which caps the dense per-batch array of the `gaussian`, `copula` and `empirical` approaches by automatically using more batches in high-dimensional settings, avoiding the `RcppArmadillo` `Cube::init(): requested size is too large` failure and often reducing runtime. For `gaussian`/`copula` the array is `n_MC_samples * n_explain * coalitions_per_batch * n_features`, and for `empirical` it is the distance array `n_train * n_explain * coalitions_per_batch`. Defaults to `1e6` elements. (branch: robustify-high-dim)
